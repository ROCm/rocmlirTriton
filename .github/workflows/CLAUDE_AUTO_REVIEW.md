...
(`.claude/skills/review-rocmlir-triton-pr`, `.claude/skills/update-pr-review`). The
...
Runs on a **self-hosted runner** (`build-only-rocmlir-triton`) because the LLM
...
| **Ensure base branch history is reachable** | Progressively deepens **both** the default branch and HEAD (the depth-1 checkout leaves HEAD parentless) until `merge-base` resolves; **hard-fails** beyond the cap. Fetches use Basic auth scoped to each `git fetch` (`x-access-token:<token>`) because the repo is private and checkout credentials are intentionally not persisted. A silent empty three-dot diff would fail-open the perimeter gate, so this step never returns "no merge base" softly. Same pattern in **Pre-fetch PR context** for the PR's actual `baseRefName`. |
...
| **Snapshot PR trust perimeter → `/tmp/pr-source`** | Preserves the PR's proposed `.claude/` + `.github/scripts/` + trusted review docs (`docs/PR_REVIEW_CHECKLIST.md`, `docs/bump_triton_version.md`) so a dispatch-path review can *read* them, even though the workspace will be overlaid with trusted versions. |
| **Overlay trusted `.claude/` + `.github/scripts/` + review docs** | Restores the default-branch versions into the workspace so the skills, sanitizer, PR review checklist, and Triton bump guide that actually *drive* the review are the trusted ones. Defense-in-depth (a PR could remove this step, but that's visible in the diff Layer 2 audits). The review docs are overlaid here so the skill's `Read(...)` calls return trusted bytes; the same files' content is also injected into the prompt itself at the snapshot step below (see [§15](#15-maintenance--sync-points)). |
| **Mint App token** | Short-lived (1h) installation token for the bot App, used for GitHub API calls and all PR writes. Generated in the same job that uses it; never written to disk; never passed to the model's env. |
...
| **Snapshot trusted review docs → step outputs** | Reads the already-overlaid trusted review docs. It drops the PR checklist H1 + following blank into `steps.snapshot_review_checklist.outputs.content`, and writes the full Triton bump guide into `steps.snapshot_review_checklist.outputs.triton_bump_content`. The prompt heredoc substitutes both outputs between matching `<BEGIN/END ...>` markers, so each file is the **single source of truth** for what the model sees -- both the prompt and `Read(...)` are driven from the same trusted bytes at the same workflow run. |
...
`/review-rocmlir-triton-pr` skill; see also [§12.2](#122-the-structured-output-contract).
...
| `snapshot_review_checklist` | `snapshot-review-checklist` | Maybe -- `docs/PR_REVIEW_CHECKLIST.md` or `docs/bump_triton_version.md` was missing/empty after the overlay, or the random heredoc-delimiter step failed (verify `openssl` is installed on the runner -- see [§14.0](#140-runner-prerequisites)). Re-run; if persistent, inspect the step log. |
...
base branch and there is no API file-count limit. The companion requests an
explicit App-token subset: `contents: read`, `issues: write`, and
`pull_requests: write`. The last two are both intentional: labels and comments
are exposed through Issues-flavored endpoints, but GitHub can still enforce PR
write access for operations on PR-backed issues. After minting the App token,
...
token requests only the subset needed by the fetch + label/comment operations;
fixed label names; perimeter file names rendered as inline-code with backticks
stripped and a 50-entry cap (so a hostile path can't inject markdown or bloat
the comment); default `GITHUB_TOKEN` is `permissions: {}`.
...
dedicated GitHub App. **This is the SAME App that services the rocMLIR
repository** -- only the *installation* is per-repo (the App itself, its
Client ID, and its private key are shared between rocMLIR and rocmlirTriton).
That is why the variable + secret names below carry the historical `rocmlir`
prefix and why the bot's posted login is the same in both repos. Each job
mints a short-lived installation token via `actions/create-github-app-token`
using the App's **Client ID** (stored as a repository *variable* — it's
non-sensitive, appears in the public install URL) and **private key** (stored
...
5. **Step 2 — run `/review-rocmlir-triton-pr`**; **Step 3 — run `/update-pr-review`**
...
- `verdict` is the model's *intended* review event (`APPROVE` /
  ...
  the `/review-rocmlir-triton-pr` skill; [§8 / Job 2](#job-2--post-has-write-no-secrets)
...
| `persist-credentials: false` + fetch-scoped Basic git auth + credential-helper git-auth | Token never written to `.git/config` where Read could dump it. |
| Layer-3 perimeter block | PR-modified security controls can't auto-run with secrets. |
| Trusted overlay of skills + scripts + review docs | Accidental (or intentional) in-PR changes to review logic, sanitizer logic, the PR review checklist tier list, or the Triton bump guide don't take effect at runtime. The review docs are also injected into the prompt at runtime by the `snapshot_review_checklist` step, which reads the already-overlaid files -- see [§15](#15-maintenance--sync-points). |
...
1. **Create the App** at **Settings → Developer settings → GitHub Apps → New
   ...
   **permissions**:
   ...
   - **Issues: Read & write** — manage repository labels and issue-label endpoints. PR label operations are PR-backed issues, so jobs that touch PR labels request this alongside Pull requests write.
   ...
   These App permissions are the installation maximum. Every workflow that
   mints this App token requests an explicit subset with
   `actions/create-github-app-token`:

   | Workflow/job | Requested App-token permissions | Why |
   |---|---|---|
   | `claude_auto_review.yml` / `review` | `checks: read`, `pull_requests: read`, `statuses: read` | Pre-fetch PR metadata, prior review comments, check runs, and commit statuses before the model runs. No write permission in the model-side job. |
   | `claude_auto_review.yml` / `post` | `pull_requests: write` | Post inline comments, thread replies/reactions, and the advisory formal review. |
   | `claude_auto_review.yml` / `cleanup` | `issues: write`, `pull_requests: write` | Remove the trigger label from the PR-backed issue; `pull_requests: write` is required for PR resources, while `issues: write` covers the Issues-label endpoint shape. |
   | `claude_auto_review_fork_notify.yml` | `issues: write`, `pull_requests: write` | Remove the trigger label and post the fork-PR notice; both scopes are requested because GitHub exposes PR labels/comments through mixed Issues and Pull requests APIs. |
   | `claude_auto_review_perimeter_banner.yml` | `contents: read`, `issues: write`, `pull_requests: write` | Fetch trusted refs, create/update the repository label, manage the PR label, and post the perimeter banner. |

   Keep the explicit requests in sync with the API calls in each job. Omitting
   `permission-*` inputs would fall back to the App installation's broader
   default token and increase blast radius if a future workflow step is
   compromised.
...
The `review` job uses `runs-on: build-only-rocmlir-triton` because the LLM gateway
...
| App-token requested permissions | The `permission-*` inputs on each `create-github-app-token` step (`review` / `post` / `cleanup` in `claude_auto_review.yml`; `fork_notify`; `perimeter_banner`) ↔ the per-job permission matrix in [§14.2](#142-create-and-install-the-bot-github-app). When a job's API calls change, update both the `permission-*` inputs and the matrix row; omitting the inputs falls back to the App's broader default token. |
...
| Review-doc content | **No sync to maintain** -- `docs/PR_REVIEW_CHECKLIST.md` and `docs/bump_triton_version.md` are the only copies. The `snapshot_review_checklist` step in `claude_auto_review.yml` reads the already-overlaid, trusted files at workflow runtime, drops the checklist H1, and writes the docs to `$GITHUB_OUTPUT`; the prompt heredoc substitutes those step outputs between matching `<BEGIN/END ...>` markers. The workflow also overlays the files into the workspace (see [§8 Job 1](#job-1--review-has-secrets-no-model-accessible-write-token)) so `Read(...)` returns the same bytes the prompt got. Drift between "the file" and "what the model reads" is impossible by construction. If you ever need to confirm the runtime substitution would produce what you expect, run locally: `tail -n +3 docs/PR_REVIEW_CHECKLIST.md | head` and `head docs/bump_triton_version.md` -- that's the content the step injects. |
...
> **Note on this port.** The rocmlirTriton pipeline is itself a port from the
> rocMLIR pipeline (see the rocMLIR repo for the original). Three narrow
...
>   `docs/PR_REVIEW_CHECKLIST.md` -- `rock::*` hardware-feature-detection
>   rule, bridge passes, fat-library + MIGraphX coordination, and the
>   **rocMLIR back-port check** -- plus the detailed Triton-bump guide in
>   `docs/bump_triton_version.md`. The five skill-name references inside
>   `claude_auto_review.yml`'s prompt heredoc are updated to match (see
>   §15 for the exact locations).
...
>   self-hosted runners with reachability to the LLM gateway; the labels
>   are kept distinct so each repo can be pointed at a separately-
>   administered runner pool.
> - The GitHub App identity is *shared* with rocMLIR (same App, same
>   Client ID + private key, same bot login `rocmlir-pr-reviewer[bot]`),
>   only the per-repo *installation* differs. The
>   `ROCMLIR_PR_REVIEWER_*` secret/variable names are intentional --
>   they reflect the App's identity, not the host repo.
>
> Everything else (sanitizer, perimeter regex, three-layer defense model,
> COMMENT-only submission, fork-notify + perimeter-banner companions,
> `post`/`fork_pr_notify` runner labels) is byte-for-byte the rocMLIR
> version.
...
**2. Runner.** `runs-on: build-only-rocmlir-triton` is needed only because the gateway
