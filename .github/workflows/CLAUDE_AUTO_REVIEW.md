...
| App-token post-mint propagation | A freshly minted App installation token can transiently 404 on a private repo before its auth propagates. Every job whose **first** App-token call could hit this runs `.github/scripts/wait_for_app_token.sh` right after `create-github-app-token` (the `review`, `post`, and `cleanup` jobs in `claude_auto_review.yml`, plus `fork_notify`). The **perimeter banner** does not call the probe: its first App-token call is the `git fetch`, which already carries its own bounded retry. The probe fails closed, so a token that never goes live aborts the job rather than proceeding. |
| Transient-error retry regex | Two hand-maintained lists of "transient, safe to retry" signatures: the `git fetch` retry in the perimeter banner (`git_fetch_with_auth`) and the API-liveness probe (`wait_for_app_token.sh`). They diverge intentionally -- git fetch and `gh api` phrase errors differently -- but both must keep matching the shared post-mint symptoms (`Repository not found` / `Not Found` / 4xx-5xx / rate limit / network). When you add a signature to one, check whether the other needs it too. Both must match via a here-string, never `printf | grep -q`, so a `grep -q` SIGPIPE under `pipefail` can't misclassify a matching (transient) error as non-retryable. |
