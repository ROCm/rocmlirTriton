// Fused GEMM test: C = exp(A + 1.0) * B
// Tests multiple chained non-zero-preserving ops as input fusion
// (both arith.addf +1.0 and math.exp are NON-zero-preserving;
//  exp(0 + 1.0) = e ≈ 2.718).
...
module attributes {rock.arch = "amdgcn-amd-amdhsa:gfx1100"} {
  func.func @rock_gemm(%arg0: tensor<10000xf16>, %arg1: tensor<10000xf16>, %arg2: tensor<10000xf16>) -> tensor<10000xf16> attributes {rock.arch = "amdgcn-amd-amdhsa:gfx1100", rock.enable_splitk_for_tuning, rock.kernel, rock.num_chiplets = 1 : i64, rock.num_cu = 96 : i64} {
    ...
    %cst = arith.constant dense<1.000000e+00> : tensor<1x100x100xf16>
    %a_add = arith.addf %0, %cst : tensor<1x100x100xf16>
    %a = math.exp %a_add : tensor<1x100x100xf16>
    %3 = rock.gemm %a * %1 : tensor<1x100x100xf16> * tensor<1x100x100xf16> -> tensor<1x100x100xf16>
    %4 = rock.store %3 to %2 by set : tensor<1x100x100xf16> -> tensor<10000xf16> to tensor<1x100x100xf16>
  ...
  func.func @host_naive_fused_gemm(%arg0: memref<10000xf16>, %arg1: memref<10000xf16>, %arg2: memref<10000xf16>) {
    ...
    scf.for %i = %c0 to %c10000 step %c1 {
      ...
      %a_add = arith.addf %a_val, %one : f32
      %a_fused = math.exp %a_add : f32
      memref.store %a_fused, %A_f32[%i] : memref<10000xf32>
    }

    // GEMM: C[g,m,n] = sum_k( exp(A+1)[g,m,k] * B[g,k,n] )
