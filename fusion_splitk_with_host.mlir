...
module attributes {rock.arch = "amdgcn-amd-amdhsa:gfx1100"} {
  ...
  func.func @rock_gemm(%arg0: tensor<10000xf16>, %arg1: tensor<10000xf16>, %inputfusion: tensor<10000xf16>, %arg2: tensor<10000xf16>, %arg3: tensor<10000xf32>) -> tensor<10000xf32> attributes {rock.arch = "amdgcn-amd-amdhsa:gfx1100", rock.enable_splitk_for_tuning, rock.kernel, rock.num_chiplets = 1 : i64, rock.num_cu = 96 : i64} {
    ...
    %4 = rock.gemm %a * %b {perf_config = "gemm:v1:64,64,64,1,1,4,16,3,2,0,0"} : tensor<1x100x100xf16> * tensor<1x100x100xf16> -> tensor<1x100x100xf16>
    %fusion = arith.addf %4, %2 : tensor<1x100x100xf16>
    %fusion_f32 = arith.extf %fusion : tensor<1x100x100xf16> to tensor<1x100x100xf32>
    %5 = rock.store %fusion_f32 to %3 by  set : tensor<1x100x100xf32> -> tensor<10000xf32> to tensor<1x100x100xf32>
  ...
  func.func @main() {
    ...
    // Run GPU kernel
    call @rock_gemm_gpu(%alloc_A, %alloc_B, %alloc_input, %alloc_ofusion, %alloc_C) : (memref<10000xf16>, memref<10000xf16>, memref<10000xf16>, memref<10000xf16>, memref<10000xf32>) -> ()

    // Run CPU reference
    call @host_naive_fused_gemm(%alloc_A_ref, %alloc_B_ref, %alloc_input_ref, %alloc_ofusion_ref, %alloc_C_ref) : (memref<10000xf16>, memref<10000xf16>, memref<10000xf16>, memref<10000xf16>, memref<10000xf32>) -> ()

    // Verify GPU output against CPU reference
    call @rock_gemm_verify2(%alloc_C, %alloc_C_ref) : (memref<10000xf32>, memref<10000xf32>) -> ()
  ...
  func.func @host_naive_fused_gemm(%arg0: memref<10000xf16>, %arg1: memref<10000xf16>, %arg2: memref<10000xf16>, %arg3: memref<10000xf16>, %arg4: memref<10000xf32>) {
    ...
    memref.copy %C_f32, %arg4 : memref<10000xf32> to memref<10000xf32>
  ...
  // ---- Verification ----
  func.func @rock_gemm_verify2(%arg0: memref<10000xf32>, %arg1: memref<10000xf32>) {
  ...
  // ---- GPU wrapper ----
  func.func @rock_gemm_gpu(%arg0: memref<10000xf16>, %arg1: memref<10000xf16>, %arg2: memref<10000xf16>, %arg3: memref<10000xf16>, %arg4: memref<10000xf32>) {
    ...
    gpu.memcpy  %memref_C, %arg4 : memref<10000xf32>, memref<10000xf32>
    ...
    %4 = bufferization.to_tensor %memref_C restrict writable : memref<10000xf32> to tensor<10000xf32>

    // Call the GPU kernel
    %5 = call @rock_gemm(%0, %1, %2, %3, %4) : (tensor<10000xf16>, tensor<10000xf16>, tensor<10000xf16>, tensor<10000xf16>, tensor<10000xf32>) -> tensor<10000xf32>

    // Copy output back
    %6 = bufferization.to_buffer %5 : tensor<10000xf32> to memref<10000xf32>
    memref.copy %6, %memref_C : memref<10000xf32> to memref<10000xf32>
    ...
    gpu.memcpy  %arg4, %memref_C : memref<10000xf32>, memref<10000xf32>
