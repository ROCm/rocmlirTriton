...
extern "C" MiirStatus miirGetExecutionDims(MiirHandle mlirHandle,
  ...
  module.walk([&](func::FuncOp funcOp) -> WalkResult {
    Attribute blockSizeAttr = funcOp->getAttr(rock::BlockSizeAttr::getMnemonic());
    if (!blockSizeAttr)
      blockSizeAttr = funcOp->getAttr(rock::BlockSizeLegacyAttr::getMnemonic());
    Attribute gridSizeAttr = funcOp->getAttr(rock::GridSizeAttr::getMnemonic());
    if (!gridSizeAttr)
      gridSizeAttr = funcOp->getAttr(rock::GridSizeLegacyAttr::getMnemonic());
    auto statusBlock = getSizeAttr(blockSizeAttr, blockSize);
    auto statusGrid = getSizeAttr(gridSizeAttr, gridSize);
  ...
  module.walk([&](gpu::BinaryOp binary) -> WalkResult {
    ...
    for (auto kernel : metadata) {
      Attribute blockSizeAttr = kernel.getAttr(rock::BlockSizeAttr::getMnemonic());
      if (!blockSizeAttr)
        blockSizeAttr = kernel.getAttr(rock::BlockSizeLegacyAttr::getMnemonic());
      Attribute gridSizeAttr = kernel.getAttr(rock::GridSizeAttr::getMnemonic());
      if (!gridSizeAttr)
        gridSizeAttr = kernel.getAttr(rock::GridSizeLegacyAttr::getMnemonic());
      auto statusBlock = getSizeAttr(blockSizeAttr, blockSize);
      auto statusGrid = getSizeAttr(gridSizeAttr, gridSize);
      if (statusBlock.succeeded() && statusGrid.succeeded()) {
        setReturn(blockSize, gridSize);
