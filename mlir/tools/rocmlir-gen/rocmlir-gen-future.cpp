...
/// Emit an element-wise copy using affine loops to avoid memrefCopy runtime
/// dependency (memref.copy lowers to memrefCopy when layouts can't be proven
/// identical at compile time)
static void emitElementWiseCopy(OpBuilder &b, Location loc, Value src,
                                Value dst) {
  auto srcType = cast<MemRefType>(src.getType());
  int64_t numElements = srcType.getNumElements();
  SmallVector<int64_t, 1> lowerBounds = {0};
  SmallVector<int64_t, 1> upperBounds = {numElements};
  SmallVector<int64_t, 1> steps = {1};
  affine::buildAffineLoopNest(
      b, loc, lowerBounds, upperBounds, steps,
      [&](OpBuilder &nestedB, Location nestedLoc, ValueRange ivs) {
        Value idx = ivs[0];
        Value val =
            affine::AffineLoadOp::create(nestedB, nestedLoc, src, ValueRange{idx});
        affine::AffineStoreOp::create(nestedB, nestedLoc, val, dst,
                                      ValueRange{idx});
      });
}
...
static void emitMemcpy(OpBuilder &b, Value src, Value dst) {
  ...
  if (srcFlatType == dstFlatType) {
    // Use element-wise copy to avoid memrefCopy runtime dependency
    emitElementWiseCopy(b, loc, srcFlat, dstFlat);
  } else {
...
/// Converts tensor element type to f32 if it's a smaller float type.
static Value ensureFloatIsF32(OpBuilder &b, Location loc, Value tensor,
  ...
  if (!isa<FloatType>(elemType) || elemType.isF32())
    return tensor;
  ...
  auto genericOp = linalg::GenericOp::create(
      ...
      [](OpBuilder &nestedB, Location nestedLoc, ValueRange args) {
        Value src = args[0];
        Value result = arith::ExtFOp::create(nestedB, nestedLoc,
                                             nestedB.getF32Type(), src);
        linalg::YieldOp::create(nestedB, nestedLoc, result);
      });
  return genericOp.getResult(0);
}

/// Helper function for linalg.generic convolution body (MAC operation)
/// Handles both float and integer types
static void convBodyBuilder(OpBuilder &b, Location loc, ValueRange blockArgs) {
  ...
  Type outputType = outputVal.getType();

  Value mul, add;
  if (isa<IntegerType>(outputType)) {
    // For integer types, extend inputs to output type before multiply
    Value inputExt = rock::createTypeConversionOp(b, loc, inputVal, outputType);
    Value filterExt =
        rock::createTypeConversionOp(b, loc, filterVal, outputType);
    mul = arith::MulIOp::create(b, loc, inputExt, filterExt);
    add = arith::AddIOp::create(b, loc, outputVal, mul);
  } else {
    mul = arith::MulFOp::create(b, loc, inputVal, filterVal);
    add = arith::AddFOp::create(b, loc, outputVal, mul);
  }
...
static Value emitGroupedConvGeneric(OpBuilder &b, Location loc,
  ...
  assert(rank >= 3 && "emitGroupedConvGeneric expects at least 3 dimensions");
...
static Value dilateTensor(OpBuilder &b, Location loc, Value tensor,
  ...
  // Create zero-filled tensor with dilated shape
  auto dilatedType =
  ...
  Value emptyDilated =
      tensor::EmptyOp::create(b, loc, dilatedType, ValueRange{});
  Value zeroDilated =
      linalg::FillOp::create(b, loc, zeroVal, emptyDilated).getResult(0);

  // Use linalg.generic to scatter original values into dilated positions
  MLIRContext *ctx = b.getContext();
  int64_t totalDims = rank;
  SmallVector<AffineExpr> d;
  for (int64_t i = 0; i < totalDims; ++i)
    d.push_back(getAffineDimExpr(i, ctx));

  // Input map: identity (read from original positions)
  SmallVector<AffineExpr> inputExprs(d.begin(), d.end());

  // Output map: non-spatial dims are identity, spatial are multiplied by stride
  SmallVector<AffineExpr> outputExprs(d.begin(), d.end());
  for (int64_t i = 0; i < dim; ++i) {
    unsigned pos = spatialDimPositions[i];
    outputExprs[pos] = d[pos] * strides[i];
  }
  ...
  SmallVector<utils::IteratorType> iteratorTypes(totalDims,
                                                 utils::IteratorType::parallel);

  return linalg::GenericOp::create(
             b, loc, dilatedType, ValueRange{tensor}, ValueRange{zeroDilated},
             indexingMaps, iteratorTypes,
             [](OpBuilder &nestedB, Location nestedLoc, ValueRange args) {
               // Simply copy the input value to the output position
               linalg::YieldOp::create(nestedB, nestedLoc, args[0]);
             })
      .getResult(0);
}
...
static Value emitBwdDataConvGeneric(OpBuilder &b, Location loc,
                                    ...
                                    ArrayRef<int64_t> filterSpatialSizes) {
  MLIRContext *ctx = b.getContext();
  int64_t rank = cast<RankedTensorType>(outputGrad.getType()).getRank();
  ...
  // Step 2: Pad the dilated output gradient at actual spatial positions
  auto dilatedType = cast<RankedTensorType>(dilatedOutputGrad.getType());
  SmallVector<OpFoldResult> lowPad(dilatedType.getRank(), b.getIndexAttr(0));
  SmallVector<OpFoldResult> highPad(dilatedType.getRank(), b.getIndexAttr(0));
  SmallVector<int64_t> paddedShape(dilatedType.getShape());

  for (int64_t i = 0; i < dim; ++i) {
    int64_t padNeeded = (filterSpatialSizes[i] - 1) * dilations[i];
    unsigned pos = outputSpatialPos[i];
    lowPad[pos] = b.getIndexAttr(padNeeded);
    highPad[pos] = b.getIndexAttr(padNeeded);
    paddedShape[pos] += 2 * padNeeded;
  }

  auto paddedType =
      RankedTensorType::get(paddedShape, dilatedType.getElementType());
  Value padValue = arith::ConstantOp::create(
      b, loc, b.getZeroAttr(dilatedType.getElementType()));
  Value paddedOutputGrad =
      tensor::PadOp::create(b, loc, paddedType, dilatedOutputGrad, lowPad,
                            highPad, padValue)
          .getResult();
...
createCPUConvWithMLIR(ModuleOp module,
  ...
  // Check for workspace (not supported in tensor version)
  ...
  if (hasWorkspace) {
    llvm::errs() << "Workspace not supported in tensor-based conv kernel\n";
    exit(1);
  }
  ...
  std::string funcName = "host_";
  funcName += rock::getNameForConvOpType(genConfig.operation.value()).str();
  ...
  bool isIntegerType = isa<IntegerType>(inputElemType);

  // Determine computation element type (f32 for floats, output type for integers)
  Type computeElemType = isIntegerType ? outputElemType : floatType;

  // Helper to expand flat tensor to logical shape, optionally converting to f32
  auto expandAndConvertToF32 = [&](Value flat,
    ...
    Value expanded =
        tensor::ExpandShapeOp::create(b, loc, logicalType, flat, allDims);

    // Convert to f32 if needed
    if (isa<FloatType>(elemType) && !elemType.isF32()) {
      auto f32Type = RankedTensorType::get(logicalShape, floatType);
      Value emptyTensor =
          tensor::EmptyOp::create(b, loc, f32Type, ValueRange{});
      AffineMap identityMap =
          AffineMap::getMultiDimIdentityMap(logicalShape.size(), ctx);
      SmallVector<utils::IteratorType> iteratorTypes(
          logicalShape.size(), utils::IteratorType::parallel);
      auto convertOp = linalg::GenericOp::create(
          b, loc, f32Type, ValueRange{expanded}, ValueRange{emptyTensor},
          ArrayRef<AffineMap>{identityMap, identityMap}, iteratorTypes,
          [](OpBuilder &nestedB, Location nestedLoc, ValueRange args) {
            Value result = arith::ExtFOp::create(nestedB, nestedLoc,
      ...
      expanded = convertOp.getResult(0);
    }
    return expanded;
  };

  // Expand tensors to logical shapes (f32 for computation)
  Value filter = expandAndConvertToF32(filterFlat, genConfig.filterDimension);
  Value input = expandAndConvertToF32(inputFlat, genConfig.inputDimension);
  Value output = expandAndConvertToF32(outputFlat, genConfig.outputDimension);
  ...
  if (needsPadding) {
    ...
    // Use empty+fill+insert_slice instead of tensor.pad to avoid memrefCopy
    // during bufferization (tensor.pad bufferizes to memref.copy between
    // different-shaped buffers which requires the memrefCopy runtime function)
    ...
    auto paddedType = RankedTensorType::get(newShape, inputElem);

    // Create empty tensor of padded size and fill with zeros
    Value emptyPadded =
        tensor::EmptyOp::create(b, loc, paddedType, ValueRange{});
    Value zeroVal = arith::ConstantOp::create(b, loc, b.getZeroAttr(inputElem));
    Value zeroPadded =
        linalg::FillOp::create(b, loc, zeroVal, emptyPadded).result();

    // Insert the original input into the padded tensor at the padding offset
    SmallVector<OpFoldResult> sizes;
    SmallVector<OpFoldResult> strides(inputType.getRank(), b.getIndexAttr(1));
    for (int64_t dim : inputType.getShape()) {
      sizes.push_back(b.getIndexAttr(dim));
    }

    input = tensor::InsertSliceOp::create(b, loc, input, zeroPadded, lowPad,
                                          sizes, strides);
  ...
  auto resultType = RankedTensorType::get(resultShape, computeElemType);
  Value zeroResult =
      arith::ConstantOp::create(b, loc, resultType, b.getZeroAttr(resultType));
  ...
  switch (genConfig.operation.value()) {
  case rock::ConvOpType::Fwd:
    result = emitGroupedConvGeneric(b, loc, resultType, input, filter,
  ...
  // Convert back to original element type if needed (only for float types
  // that were promoted to f32 for computation)
  ...
  Type finalElemType = resultFlatTensorType.getElementType();
  if (!isIntegerType && finalElemType != floatType) {
    auto resultOrigType = RankedTensorType::get(finalResultShape, finalElemType);
    Value emptyConvert =
...
createCPUConvFunc(ModuleOp module,
  ...
  // Check for workspace (not supported in tensor-based version)
  rock::ConvGenerator convGenerator(genConfig);
  bool hasWorkspace = false;
  if (failed(convGenerator.hasWorkspace(b, hasWorkspace))) {
    assert(genConfig.operation.value() == rock::ConvOpType::Fwd);
  ...
  if (hasWorkspace) {
    llvm::errs() << "Workspace not supported in tensor-based CPU conv kernel\n";
    exit(1);
  }
  ...
  // Create the tensor-based kernel and call it from this memref wrapper
  func::FuncOp tensorKernel = createCPUConvWithMLIR(module, genConfig);

  Block *block = func.addEntryBlock();
  b.setInsertionPointToStart(block);

  Value filterArg = block->getArgument(0);
  Value inputArg = block->getArgument(1);
  Value outputArg = block->getArgument(2);

  // Convert memref args to tensors
  Value filterTensor = rock::getAsTensor(b, loc, filterArg);
  Value inputTensor = rock::getAsTensor(b, loc, inputArg);
  Value outputTensor = rock::getAsTensor(b, loc, outputArg);

  // Call the tensor-based kernel
  Value result = func::CallOp::create(b, loc, tensorKernel,
                                      ValueRange{filterTensor, inputTensor,
                                                 outputTensor})
                     .getResult(0);

  // Determine the output memref type and destination based on operation type
  MemRefType resultMemRefType;
  Value destArg;
  switch (genConfig.operation.value()) {
  case rock::ConvOpType::Fwd:
    resultMemRefType = outputType;
    destArg = outputArg;
    break;
  case rock::ConvOpType::BwdData:
    resultMemRefType = inputType;
    destArg = inputArg;
    break;
  case rock::ConvOpType::BwdWeight:
    resultMemRefType = filterType;
    destArg = filterArg;
    break;
  }

  // Copy result back to the appropriate output memref
  // Use element-wise copy instead of memref.copy to avoid memrefCopy runtime
  // dependency (memref.copy lowers to memrefCopy when layouts can't be proven
  // identical at compile time)
  Value resultBuffer =
      bufferization::ToBufferOp::create(b, loc, resultMemRefType, result);

  int64_t numElements = resultMemRefType.getNumElements();
  SmallVector<int64_t, 1> lowerBounds = {0};
  SmallVector<int64_t, 1> upperBounds = {numElements};
  SmallVector<int64_t, 1> steps = {1};
  affine::buildAffineLoopNest(
      ...
      [&](OpBuilder &nestedB, Location nestedLoc, ValueRange ivs) {
        Value idx = ivs[0];
        Value val = affine::AffineLoadOp::create(nestedB, nestedLoc,
                                                 resultBuffer, ValueRange{idx});
        affine::AffineStoreOp::create(nestedB, nestedLoc, val, destArg,
                                      ValueRange{idx});
      });
...
static func::FuncOp createCpuGemmKernelWithMlir(ModuleOp module,
  ...
  Value aExpVal = expandTensorArg(aVal, argTypes[0]),
        ...
        cExpVal = expandTensorArg(cVal, argTypes[2]);
...
createCpuConvElementwiseGemmKernelWithMlir(ModuleOp module,
  ...
  // Use element-wise copy to avoid memrefCopy runtime dependency
  emitElementWiseCopy(builder, loc, flatResultMemref, output);
...
static func::FuncOp createCpuAttentionKernelWithMlir(ModuleOp module,
  ...
  if (returnLSE) {
    ...
    // Use element-wise copy to avoid memrefCopy runtime dependency
    emitElementWiseCopy(builder, loc, flatLseMemref, lseOut);
  }
  ...
  for (auto [resultIdx, result] : llvm::enumerate(callOp.getResults())) {
    if (resultIdx < outputIndices.size()) {
      ...
      Value resultMemref =
          bufferization::ToBufferOp::create(b, loc, outMemrefType, result);
      // Use element-wise copy to avoid memrefCopy runtime dependency
      emitElementWiseCopy(b, loc, resultMemref, memrefArgs[outIdx]);
    }
  }
}
