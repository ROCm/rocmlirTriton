...
static bool isDynamicDim(StringRef name) {
  return llvm::is_contained(dynamicDims, name);
}

/// `type` with the dimensions named in `dimNames` that are in --dynamic-dims
/// made dynamic.
static Type makeDynamicType(Type type, ArrayRef<StringRef> dimNames) {
  auto shaped = dyn_cast<RankedTensorType>(type);
  if (!shaped || dynamicDims.empty())
    return type;
  SmallVector<int64_t> shape(shaped.getShape());
  for (auto [dim, name] : llvm::zip(shape, dimNames))
    if (isDynamicDim(name))
      dim = ShapedType::kDynamic;
  return shaped.clone(shape);
}

/// For the dynamic kernel generated by this run: the flat static argument
/// types the host harness allocates, as for a static kernel, and the static
/// logical shapes it expands them to before casting to the kernel's types.
static SmallVector<Type, 8> dynamicKernelHostTypes;
static SmallVector<Type, 8> dynamicKernelLogicalTypes;

/// Records the host types of a dynamic kernel taking `logicalTypes`.
static void setDynamicKernelHostTypes(ArrayRef<Type> logicalTypes) {
  dynamicKernelLogicalTypes.assign(logicalTypes.begin(), logicalTypes.end());
  dynamicKernelHostTypes =
...
/// Checks the --dynamic-dims names against those the operation supports.
  ...
  for (StringRef name : dynamicDims) {
    if (!llvm::is_contained(valid, name)) {
      llvm::errs() << "invalid --dynamic-dims entry '" << name
                   << "'; expected one of: " << llvm::join(valid, ",")
                   << "\n";
      return failure();
    }
  }
    ...
    for (const auto &kernel : kernels) {
      ...
      if (expectsTensors) {
        ...
        for (auto [resultIdx, result] : llvm::enumerate(callOp.getResults())) {
          ...
          auto outMemrefType = cast<MemRefType>(kernelMem[outIdx].getType());
          Value resultMemref =
              bufferization::ToBufferOp::create(b, loc, outMemrefType, result);
          memref::CopyOp::create(b, loc, resultMemref, kernelMem[outIdx]);
        }
      } else {
...
static func::FuncOp createGpuGemmKernel(ModuleOp module,
  ...
  // dynamic dims as `?`, and stores the GEMM straight into C.
  if (!dynamicDims.empty()) {
    SmallVector<Type> dynArgTypes = {makeDynamicType(argTypes[0], allArgNames[0]),
                                     makeDynamicType(argTypes[1], allArgNames[1]),
                                     makeDynamicType(cType, cDimNames)};
    setDynamicKernelHostTypes({argTypes[0], argTypes[1], cType});
    auto func = func::FuncOp::create(
        b, loc, kernelName, b.getFunctionType(dynArgTypes, {dynArgTypes[2]}),
        funcAttrs);
    Block *block = func.addEntryBlock();
    b.setInsertionPointToStart(block);
    auto gemm = rock::GemmOp::create(
        b, loc, dynArgTypes[2], func.getArgument(0), func.getArgument(1),
        /*aScale=*/nullptr, /*bScale=*/nullptr, transposeA, transposeB,
        transposeO, transposeScaleA, transposeScaleB,
        /*scaleBlockSize=*/nullptr, /*params=*/nullptr);
    if (!params.perfConfig.empty())
      gemm->setAttr("perf_config", b.getStringAttr(params.perfConfig));
    Value stored = rock::StoreOp::create(
        b, loc, dynArgTypes[2], gemm.getResult(), func.getArgument(2),
...
static void generateKernel(MLIRContext *context, GenParams &genParams,
  ...
  if (failed(detectMissingArguments()) || failed(validateDynamicDims())) {
    exit(1);
  }
