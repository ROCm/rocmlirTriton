...
LogicalResult ConvConverter<ConvType>::matchAndRewrite(
  ...
  // TOSA requires exact divisibility by stride. Adjust padding and slice
  // input if needed.
  if (!isBwdDataConvOp) {
    OpBuilder::InsertionGuard guard(rewriter);
    rewriter.setInsertionPoint(cop);
    input = rock::tosa::adjustConvPadding(rewriter, loc, input, filter, pads,
                                          strides, dilations);
...
LogicalResult DotConverter<DotType>::matchAndRewrite(
  ...
  if constexpr (std::is_same_v<DotType, migraphx::QuantDotOp>) {
    if (hasScales) {
      ...
      auto castScaleToMXFP = [&](Value v, Type elemTy) {
        if (elemTy == mxfpScaleType)
          return v;
        return rewriter.createOrFold<tosa::CastOp>(
            loc, cast<RankedTensorType>(v.getType()).clone(mxfpScaleType), v);
      };
      scaleAUnbroadcasted =
          castScaleToMXFP(scaleAUnbroadcasted, scaleAElementType);
      ...
      scaleBUnbroadcasted =
          castScaleToMXFP(scaleBUnbroadcasted, scaleBElementType);
