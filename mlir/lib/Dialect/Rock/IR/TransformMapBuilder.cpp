  ...
  for (const TransformAttr transform : transforms) {
    ...
    if (type == TransformType::PassThrough) {
      ...
      for (uint32_t i = 0, e = lowerDims.size(); i < e; ++i) {
        ...
        // We intentionally emit `(x floordiv stride) mod size` instead of
        // chaining a remainder through `(x mod prevStride) floordiv stride`
        // because that way, Triton's AxisInfo is able to analyze the IR chain
        // better and properly vectorize the code.
