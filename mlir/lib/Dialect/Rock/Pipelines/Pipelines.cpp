...
static void buildHostLoweringPipeline(mlir::OpPassManager &pm) {
  ...
  // Bufferize remaining tensor operations in the module (e.g., in main function).
  // The CPU verifier functions are already lowered to LLVM, so this only affects
  // the remaining func.func operations (main, GPU wrappers).
  // This converts bufferization.to_tensor/to_buffer ops to memref operations.
  // We use copyBeforeWrite=true to skip analysis (avoids issues with non-restrict
  // to_tensor ops that may be generated by earlier passes).
  bufferization::OneShotBufferizePassOptions bufOpts;
  bufOpts.bufferizeFunctionBoundaries = true;
  bufOpts.functionBoundaryTypeConversion =
      bufferization::LayoutMapOption::IdentityLayoutMap;
  bufOpts.copyBeforeWrite = true;
  pm.addPass(bufferization::createOneShotBufferizePass(bufOpts));
