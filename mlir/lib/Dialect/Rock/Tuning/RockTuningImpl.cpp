...
// Non-power-of-two kPerBlock: all non-pow2 divisors of K in [minBaseK, maxK].
// EXPERIMENT: removed the window and popcount-2 filters to explore the full
// space of non-pow2 divisors.
...
windowDividingKPerBlock(int64_t k, uint32_t mPerBlock, uint32_t nPerBlock,
  ...
  if (k <= 0)
    return candidates;
  uint32_t hi = std::min<uint32_t>(maxK, static_cast<uint32_t>(k));
  for (uint32_t d = minBaseK; d <= hi; ++d) {
    if (k % d == 0 && !llvm::isPowerOf2_32(d))
