...
static void createGemmTuningRangeQuick(TuningParamSet *newSpace,
  ...
  for (GemmParamsAttr param : tuningInfo.getTuningParameters(
           ...
           info.problemHash, info.quantBlockSize, info.aScaleType,
           info.bScaleType)) {
...
LogicalResult getTuningProblemStr(ModuleOp mod, SmallVectorImpl<char> &out) {
  return visitPrimaryTuningOp<LogicalResult>(mod, [&](auto tuningOp) {
    if (failed(
            serializeTuningProblem(tuningOp, TuningProblemFormat::PerfDb, out)))
      return failure();
