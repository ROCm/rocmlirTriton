...
static bool lowerSubByteShiftConstant(arith::ConstantOp constOp) {
  ...
  if (!tensorType || tensorType.getRank() != 2) {
    constOp.emitError("rock.sub_byte_shift requires a rank-2 tensor, got ")
        << constOp.getType();
    return false;
  }
  if (!tensorType.getElementType().isInteger(8)) {
    constOp.emitError("rock.sub_byte_shift requires i8 element type, got ")
        << tensorType.getElementType();
    return false;
  }

  auto axisAttr = dyn_cast_or_null<IntegerAttr>(dictAttr.get("axis"));
  auto halfPeriodAttr =
      dyn_cast_or_null<IntegerAttr>(dictAttr.get("halfPeriod"));
  auto v0Attr = dyn_cast_or_null<IntegerAttr>(dictAttr.get("v0"));
  auto v1Attr = dyn_cast_or_null<IntegerAttr>(dictAttr.get("v1"));
  if (!axisAttr || !halfPeriodAttr || !v0Attr || !v1Attr) {
    constOp.emitError("rock.sub_byte_shift missing or ill-typed entries; "
                      "expected integer attrs {axis, halfPeriod, v0, v1}");
    return false;
  }

  int64_t axis = axisAttr.getInt();
  if (axis < 0 || axis > 1) {
    constOp.emitError("rock.sub_byte_shift axis must be 0 or 1, got ")
        << axis;
    return false;
  }
  int64_t halfPeriod = halfPeriodAttr.getInt();
  if (halfPeriod <= 0) {
    constOp.emitError("rock.sub_byte_shift halfPeriod must be > 0, got ")
        << halfPeriod;
    return false;
  }
  int8_t v0 = static_cast<int8_t>(v0Attr.getInt());
  int8_t v1 = static_cast<int8_t>(v1Attr.getInt());

  auto shape = tensorType.getShape();
  int64_t axisLen = shape[axis];
  int64_t otherAxis = 1 - axis;
