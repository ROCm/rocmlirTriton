...
sliceSplitKVFromBatch(PatternRewriter &rewriter, Location loc, Value tensor,
  ...
  if (!trailingDimNames.empty()) {
    SmallVector<uint32_t> trailingUpperDims;
  ...
  sliceBuilder.dropDimAtIndex("splitKV", /*index=*/0);
...
struct DetectFlashDecodingPattern : public OpRewritePattern<AttentionOp> {
  ...
  LogicalResult matchAndRewrite(AttentionOp op,
    ...
    // The optional per-batch tensors carry the same merged batch dimension as
    // Q, K and V, so they have to lose the splitKV factor along with them.
    ...
    auto newOp = rock::AttentionOp::create(
        ...
        maybeNewLastValidKVIndex.value(), maybeNewPrefixOffset.value(),
