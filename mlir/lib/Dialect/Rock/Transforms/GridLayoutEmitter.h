...
struct GridLayoutInfo {
  ...
  int64_t numCU;
  int64_t numChiplets;
  Type aInputType; // A operand load element type
  Type bInputType; // B operand load element type
  int64_t gridGroupSize;
  // Tile sizes and whole-matrix byte counts for the cache-residency (1-D
  // GROUP_SIZE_M) groupSizeM heuristic.
  int64_t mPerBlock;   // M-block tile (elements)
  int64_t nPerBlock;   // N-block tile (elements)
  ...
  int64_t l2Bytes;     // L2 shared by the concurrent workgroups (band height)
  int64_t llcBytes;    // last-level cache; governs read-once DRAM traffic
};
