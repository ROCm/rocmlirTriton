...
void RockMaskNonZeroPreservingFusionsPass::runOnOperation() {
  ...
  for (auto &[leaf, masks] : leafMasks) {
    ...
    //
    // Filler value: must be the algebraic identity of the leaf's downstream
    // consumer. tt.dot and sum-reduce want zero; max-reduce wants -inf;
    // min-reduce wants +inf. See chooseFillerKind() for caveats.
