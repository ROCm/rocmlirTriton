...
// Keep dependents before their providers: linking OCML can introduce OCKL
// references that must be visible when deciding whether to link OCKL.
...
bool linkDeviceLibraryModule(llvm::Module &module,
  ...
  llvm::StringSet<> externalFns;
  for (llvm::Function &fn : libMod->functions()) {
    if (!fn.isDeclaration())
      externalFns.insert(fn.getName());
  }
  ...
  for (llvm::Function &fn : module.functions()) {
    if (externalFns.contains(fn.getName()))
