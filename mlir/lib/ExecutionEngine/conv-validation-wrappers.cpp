...
// Per-operation timing utilities for CPU verifier
// Operation IDs must match CpuVerifierOpId enum in rocmlir-gen.cpp
...
static const char* getOpName(int32_t opId) {
  switch (opId) {
    case CpuOpExtF_Filter: return "extf (filter bf16->f32)";
    case CpuOpExtF_Input: return "extf (input bf16->f32)";
    case CpuOpExtF_Output: return "extf (output bf16->f32)";
    case CpuOpPadInput: return "pad (input)";
    case CpuOpFill: return "fill (zero init)";
    case CpuOpDilate: return "generate (dilate)";
    case CpuOpPadOrCrop: return "pad/crop (bwd_data)";
    case CpuOpFlipFilter: return "generate (flip filter)";
    case CpuOpConvGeneric: return "generic (convolution)";
    case CpuOpTruncF: return "truncf (f32->bf16)";
...
static std::array<OpTimerStats, CpuOpTotal> opTimerStats;
static std::chrono::steady_clock::time_point opTimerStartPoint;
static int32_t currentOpId = -1;

extern "C" void cpuOpTimerStart(int32_t opId) {
  currentOpId = opId;
  opTimerStartPoint = std::chrono::steady_clock::now();
}

extern "C" void cpuOpTimerStop() {
  auto endPoint = std::chrono::steady_clock::now();
  auto elapsed = std::chrono::duration<double, std::milli>(endPoint - opTimerStartPoint).count();
  
  if (currentOpId >= 0 && currentOpId < CpuOpTotal) {
    opTimerStats[currentOpId].totalTime += elapsed;
    opTimerStats[currentOpId].count++;
  }
}

extern "C" void cpuOpTimerPrintStats() {
  bool hasStats = false;
  for (int i = 0; i < CpuOpTotal; ++i) {
    if (opTimerStats[i].count > 0) {
      hasStats = true;
      break;
    }
  }
  ...
  for (int i = 0; i < CpuOpTotal; ++i) {
    const auto& stats = opTimerStats[i];
    if (stats.count > 0) {
      double avg = stats.totalTime / stats.count;
      printf("%-30s %12.3f %8lld %12.3f\n", getOpName(i), stats.totalTime, 
             (long long)stats.count, avg);
      grandTotal += stats.totalTime;
    }
  }
  printf("%-30s %12s %8s %12s\n", "------------------------------", "------------", "--------", "------------");
  printf("%-30s %12.3f\n", "Total", grandTotal);
  ...
  for (int i = 0; i < CpuOpTotal; ++i) {
    opTimerStats[i] = OpTimerStats{0.0, 0};
  }
}
