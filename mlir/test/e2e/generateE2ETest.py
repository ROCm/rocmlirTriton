...
if __name__ == '__main__':
    ...
    # list of axis-value combinations using the same semantics as
    # [[suite.test.exclude]]: AND across [[require.match]] blocks, OR within
    # the values of a single block.
    requires = []
    for req in toml_dict.get("require", []):
        feature = req["feature"]
        match_combinations = generate_option_list(axis_prefixes, req, "match", "values")
        requires.append((feature, match_combinations))
    ...
    for suite in toml_dict["suite"]:
        ...
        for test in suite["test"]:
            ...
            for opt in combinations:
                ...
                if skip:
                    continue
                # Collect lit features required by any [[require]] block whose
                # match combinations are a subset of this test's axis values.
                required_features = []
                for feature, match_combinations in requires:
                    if feature in required_features:
                        continue
                    for match in match_combinations:
                        if all(x in opt for x in match):
                ...
                with open(output, 'w') as f:
                    ...
                    if required_features:
                        f.write(f"// REQUIRES: {', '.join(required_features)}\n")
