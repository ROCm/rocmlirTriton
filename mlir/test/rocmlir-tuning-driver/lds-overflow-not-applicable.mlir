...
//
// The chosen tile (mPerBlock=nPerBlock=kPerBlock=256, kpack=1, f32) makes
// Triton allocate 524288 bytes (512 KB) of shared memory, which exceeds the
// LDS budget on every currently supported architecture: gfx90a/942/1030/
...
// RUN:     --benchmark-config="gemm:v1:256,256,256,1,1,4,16,1,2,0,0" 2>&1 \
// RUN: | FileCheck %s \
...
// CHECK: gemm:v1:256,256,256,1,1,4,16,1,2,0,0{{[[:space:]]+}}N/A
