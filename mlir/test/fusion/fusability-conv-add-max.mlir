...
module {
  func.func @mlir_convolution_add_relu(%arg0: tensor<64x1x1x1xf32>, %arg1: tensor<1x256x56x56xf32>, %arg2: tensor<64x256x1x1xf32>, %arg3: tensor<1x64x56x56xf32>) -> tensor<1x64x56x56xf32> attributes {rock.enable_splitk_for_tuning, rock.kernel, rock.arch = "amdgcn-amd-amdhsa:gfx90a:sramecc+:xnack-"} {
    ...
    %4 = rock.transform %arg3 by <affine_map<(d0, d1, d2, d3, d4) -> (d0, d1 * 64 + d2, d3, d4)> by [<PassThrough ["n", "h", "w"] at [0, 3, 4] -> ["n", "h", "w"] at [0, 2, 3]>, <Unmerge{1, 64} ["g", "k"] at [1, 2] -> ["k"] at [1]>] bounds = [1, 1, 64, 56, 56] -> [1, 64, 56, 56]> : tensor<1x64x56x56xf32> to tensor<1x1x64x56x56xf32>
    %5 = rock.conv(%3, %2, %4) {arch = "amdgcn-amd-amdhsa:gfx90a:sramecc+:xnack-", dilations = [1 : index, 1 : index], filter_layout = ["g", "k", "c", "y", "x"], input_layout = ["ni", "gi", "ci", "hi", "wi"], output_layout = ["no", "go", "ko", "ho", "wo"], padding = [0 : index, 0 : index, 0 : index, 0 : index], strides = [1 : index, 1 : index]} : tensor<1x64x256x1x1xf32>, tensor<1x1x256x56x56xf32>, tensor<1x1x64x56x56xf32> -> tensor<1x1x64x56x56xf32>
    %6 = rock.transform %5 by <affine_map<(d0, d1, d2) -> (0, d0 floordiv 64, d0 mod 64, d1, d2)> by [<Merge{1, 64} ["dim0"] at [0] -> ["col0", "col1"] at [0, 1]>, <PassThrough ["dim1", "dim2"] at [1, 2] -> ["dim1", "dim2"] at [2, 3]>] bounds = [64, 56, 56] -> [1, 1, 64, 56, 56]> : tensor<1x1x64x56x56xf32> to tensor<64x56x56xf32>
