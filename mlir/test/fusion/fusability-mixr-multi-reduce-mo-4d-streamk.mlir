// Stream-K uses the v5 perf-config format: field 12 is streamKMultiple (>= 1
// enables stream-K, 0 disables it), while splitKFactor (field 8) stays 1 since
// the two are mutually exclusive. Both the stream-K-enabled and disabled configs
// must report the same fusibility as the plain/split-k paths for this
// conv + bias-add + reduce(sum) fusion.

// RUN: rocmlir-gen -emit-module-fusibility-for=gemm:v5:64,64,16,1,1,4,16,1,2,0,0,1,-1,-1,-1,-1,-1,-1 - < %s | FileCheck %s --check-prefixes=CHECK-STREAMK
// CHECK-STREAMK: fusible:1
// RUN: rocmlir-gen -emit-module-fusibility-for=gemm:v5:64,64,16,1,1,4,16,1,2,0,0,0,-1,-1,-1,-1,-1,-1 - < %s | FileCheck %s --check-prefixes=CHECK-NONSTREAMK
// CHECK-NONSTREAMK: fusible:1
