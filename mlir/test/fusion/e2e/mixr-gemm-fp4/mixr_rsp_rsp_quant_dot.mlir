...
// RUN: rocmlir-gen -fut mlir_quant_dot_fp4 --arch %arch %s --clone-harness | rocmlir-driver -host-pipeline=migraphx,highlevel -kernel-pipeline=migraphx,highlevel | rocmlir-gen -ph -fut mlir_quant_dot_fp4 --verifier clone - | rocmlir-driver -c | mlir-runner -O2 --shared-libs=%linalg_test_lib_dir/%shlibprefixmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/%shlibprefixconv-validation-wrappers%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_runner_utils%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_float16_utils%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_c_runner_utils%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_async_runtime%shlibext --entry-point-result=void | FileCheck %s
// CHECK: [1 1 1]

module {
  func.func @mlir_quant_dot_fp4(%arg0: !migraphx.shaped<1x256x768xf4E2M1FN, 196608x768x1>, %arg1: !migraphx.shaped<768x50272xf4E2M1FN, 50272x1>, %arg2: !migraphx.shaped<1x8x1x768xf32, 6144x768x768x1>, %arg3: !migraphx.shaped<1x768x1571x1xf32, 1206528x1571x1x1>) -> !migraphx.shaped<1x256x50272xf32, 12869632x50272x1> attributes {rock.kernel} {
