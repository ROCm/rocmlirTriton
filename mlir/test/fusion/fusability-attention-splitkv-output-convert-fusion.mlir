...
// Ported from rocMLIR, which tracks it as a regression test for
// https://github.com/ROCm/rocMLIR/issues/2376

// RUN: rocmlir-gen -emit-module-fusibility-for=attn:v1:64,64,16,1,1,4,16,1,2,0,0 - < %s | FileCheck %s
// CHECK: fusible:1

module {
  func.func @attn_splitkv_output_extf_fusible(%q: tensor<1x1024x64xf16>, %k: tensor<1x64x1024xf16>, %v: tensor<1x1024x64xf16>, %out: tensor<4x1024x64xf32>) -> tensor<4x1024x64xf32> attributes {rock.kernel, rock.arch = "amdgcn-amd-amdhsa:gfx942:sramecc+:xnack-"} {
    ...
    %stored = rock.store %widened to %out by set : tensor<4x1024x64xf32> -> tensor<4x1024x64xf32> to tensor<4x1024x64xf32>
