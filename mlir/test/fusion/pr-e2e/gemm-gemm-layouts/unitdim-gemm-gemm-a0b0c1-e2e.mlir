...
// RUN: rocmlir-gen --clone-harness -arch %arch -fut test %s | rocmlir-driver -kernel-pipeline migraphx,highlevel -host-pipeline migraphx,highlevel -arch %arch | rocmlir-gen -ph -verifier clone -fut test - | rocmlir-driver -c --mlir-print-ir-after=tritongpu-coalesce -o /dev/null 2>&1 | FileCheck %s --check-prefix=NARROWED

// CLONE: [1 1 1]

// EMITKEY: -t f32 -transA false -transB false -transC true -transO false -g 1 -m 32 -n 64 -k 1 -gemmO 8

// The k=1 operands are read once and broadcast, not as redundant full tiles.
// NARROWED-DAG: tt.load {{.*}} : tensor<{{[0-9]+}}x1x!tt.ptr<f32>
// NARROWED-DAG: tt.load {{.*}} : tensor<1x{{[0-9]+}}x!tt.ptr<f32>
