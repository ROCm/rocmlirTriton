...
// RUN: rocmlir-gen --clone-harness -arch gfx1200 -fut test %s | rocmlir-driver -kernel-pipeline migraphx,highlevel -host-pipeline migraphx,highlevel -arch gfx1200 | rocmlir-gen -ph -verifier clone -fut test - | rocmlir-driver -c --debug-only=rock-gridwise-to-blockwise -o /dev/null 2>&1 | FileCheck %s --check-prefix=VECTORIZATION
