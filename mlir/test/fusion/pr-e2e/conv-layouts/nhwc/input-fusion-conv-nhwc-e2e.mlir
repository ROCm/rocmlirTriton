// RUN: rocmlir-gen --clone-harness -arch %arch -fut test %s | rocmlir-driver -pipeline migraphx,highlevel -targets %arch | rocmlir-gen --emit-tuning-key - | FileCheck %s  --check-prefixes=EMITKEY
// RUN: rocmlir-gen --clone-harness -arch %arch -fut test %s | rocmlir-driver -pipeline migraphx,highlevel -targets %arch | rocmlir-gen -ph -relDiff_threshold 0.09 -absDiff_threshold 1 -RMS_threshold 0.05 -verifier clone -fut test_wrapper - | rocmlir-driver -pipeline full | xmir-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext,%linalg_test_lib_dir/libmlir_c_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_async_runtime%shlibext --entry-point-result=void | FileCheck %s --check-prefix=CLONE
// RUN: rocmlir-gen --clone-harness -arch gfx942 -fut test %s | rocmlir-driver -pipeline migraphx,highlevel -targets gfx942 | rocmlir-gen -ph -verifier clone -fut test_wrapper - | rocmlir-driver -pipeline full --debug-only=rock-gridwise-to-blockwise -o /dev/null 2>&1 | FileCheck %s --check-prefix=VECTORIZATION
// CLONE: [1 1 1]
