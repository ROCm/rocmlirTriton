// RUN: rocmlir-gen --clone-harness -arch %arch -fut mlir_convert_convolution %s | rocmlir-driver  -kernel-pipeline migraphx,highlevel -host-pipeline migraphx,highlevel -arch %arch | rocmlir-gen -ph -verifier clone -fut mlir_convert_convolution - | rocmlir-driver -c | mlir-runner --shared-libs=%linalg_test_lib_dir/%shlibprefixmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/%shlibprefixconv-validation-wrappers%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_runner_utils%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_c_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=CLONE
// CLONE: [1 1 1]

module {
  func.func @mlir_convert_convolution(%arg0: !migraphx.shaped<1x1x128x128x128xf32, 2097152x2097152x16384x128x1>, %arg1: !migraphx.shaped<32x1x3x3x3xf16, 27x27x9x3x1>) -> !migraphx.shaped<1x32x128x128x128xf16, 67108864x2097152x16384x128x1> attributes {rock.kernel} {
