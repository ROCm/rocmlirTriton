...
//
// E2E check for a TRANSPOSED output extf (f16 -> f32) on splitKV > 1.
//
// rocmlirTriton reports fusible:1 here because the transpose is an explicit
// rock.transform view with the layout carried by rock.store, where upstream
// rocMLIR reports fusible:0 for the linalg.generic indexing-map form. This
// pins down that the transpose costs no accuracy: the verifier statistics are
// identical to the non-transposed variant next to it.
//
// See that test for why rtol is loosened from the allclose default.

// RUN: rocmlir-gen -fut mlir_attention --arch %arch --clone-harness %s | rocmlir-driver -kernel-pipeline=migraphx,highlevel -host-pipeline=migraphx,highlevel | rocmlir-gen -ph -rand 1 -rand_type float -fut mlir_attention --rtol 0.01 -relDiff_threshold 0.0005 -RMS_threshold 0.002 --verifier clone - | rocmlir-driver -c | mlir-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext,%linalg_test_lib_dir/libmlir_c_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_async_runtime%shlibext --entry-point-result=void | FileCheck %s
// CHECK: [1 1 1]
// CHECK-NEXT: [1 1 1]

module {
  func.func @mlir_attention(%arg0: !migraphx.shaped<1x256x256xf16, 65536x256x1>, %arg1: !migraphx.shaped<1x256x256xf16, 65536x256x1>, %arg2: !migraphx.shaped<1x256x256xf16, 65536x256x1>) -> (!migraphx.shaped<1x2x256x256xf32, 131072x65536x1x256>, !migraphx.shaped<1x2x256x1xf32, 512x256x1x1>) attributes {rock.kernel = "mixr"} {
    ...
    %24 = migraphx.transpose %20 {permutation = [0, 1, 3, 2]} : <1x2x256x256xf16, 131072x65536x256x1> -> <1x2x256x256xf16, 131072x65536x1x256>
    %21 = migraphx.convert %24 {target_type = 2 : i64} : <1x2x256x256xf16, 131072x65536x1x256> to <1x2x256x256xf32, 131072x65536x1x256>
    ...
    return %21, %23 : !migraphx.shaped<1x2x256x256xf32, 131072x65536x1x256>, !migraphx.shaped<1x2x256x1xf32, 512x256x1x1>
  }
}
