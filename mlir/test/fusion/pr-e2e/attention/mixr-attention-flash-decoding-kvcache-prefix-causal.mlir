// XFAIL: *
// Ported from rocMLIR, currently expected to fail. rock-gridwise-attn-to-blockwise
// rejects KV-cache flash decoding when a pre-softmax fusion is present: the
// split-KV view built for the GEMM0 output is derived from the chain's lower
// shape and then prepended above a chain whose upper shape is already split, so
// it is applied twice and the verifier reports e.g. "input shape=64, 5, 512,
// lower bounds=32, 5, 1024". Split-KV with a pre-softmax fusion but no KV cache
// lowers correctly today (see mixr-attention-flash-decoding-3d-presoftmax-fusion),
// so the mismatch is specific to the KV-cache view chain.

// RUN: rocmlir-gen -fut mlir_attention --arch %arch --clone-harness %s | rocmlir-driver -kernel-pipeline=migraphx,highlevel -host-pipeline=migraphx,highlevel | rocmlir-gen -ph -fut mlir_attention -relDiff_threshold 0.00001 -rand_min_int 0 -rand_max_int 4 -rand_type_int_for_inputs=2,4 --verifier clone - -pr | rocmlir-driver -c | mlir-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext,%linalg_test_lib_dir/libmlir_c_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_async_runtime%shlibext --entry-point-result=void | FileCheck %s
