...
// RUN: rocmlir-gen --arch %arch --operation attention -last_valid_kv_index=2 -sliding_window_look_back=1 --causal -return_lse -split_kv=8 -seq_len_q 1 -seq_len_k 64 -head_dim_qk 32 -head_dim_v 32 -t f32 -rand 1 -rand_type float -pv -pr -pvr \
// RUN: | rocmlir-driver --host-pipeline=highlevel \
// RUN: | rocmlir-driver -c \
// RUN: | mlir-runner -O2 --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext --entry-point-result=void \
...
// RUN: rocmlir-gen --arch %arch --operation attention -last_valid_kv_index=2 -sliding_window_look_back=1 --causal -return_lse -split_kv=8 -seq_len_q 4 -seq_len_k 64 -head_dim_qk 32 -head_dim_v 32 -t f32 -rand 1 -rand_type float -pv -pr -pvr \
// RUN: | rocmlir-driver --host-pipeline=highlevel \
// RUN: | rocmlir-driver -c \
// RUN: | mlir-runner -O2 --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext --entry-point-result=void \
// RUN: | FileCheck %s --check-prefix=MIXED-SPLIT
...
// RUN: rocmlir-gen --arch %arch --operation attention -last_valid_kv_index=2 -sliding_window_look_back=1 --causal -return_lse -split_kv=8 -seq_len_q 4 -seq_len_k 64 -head_dim_qk 32 -head_dim_v 32 -t f16 -rand 1 -rand_type float -pv -pr -pvr \
// RUN: | rocmlir-driver --host-pipeline=highlevel \
// RUN: | rocmlir-driver -c \
// RUN: | mlir-runner -O2 --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext --entry-point-result=void \
// RUN: | FileCheck %s --check-prefix=F16-SPLIT
// RUN: rocmlir-gen --arch %arch --operation attention --causal -return_lse -split_kv=8 -seq_len_q 256 -seq_len_k 512 -head_dim_qk 32 -head_dim_v 32 -t f32 -rand 1 -rand_type float -pv \
// RUN: | rocmlir-driver --host-pipeline=highlevel \
// RUN: | rocmlir-driver -c \
// RUN: | mlir-runner -O2 --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext --entry-point-result=void \
// RUN: | FileCheck %s --check-prefix=MULTIBLOCK-SPLIT
...
// path exercises host recombination of fully masked partial results. The
// mixed cases use q=0 as a fully masked row and later rows as valid rows.
// The multiblock case spans several query and key tiles so host recombination
