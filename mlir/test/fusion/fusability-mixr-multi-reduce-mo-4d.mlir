...
module {
  func.func @mlir_convolution_multi_reduce(%arg0: tensor<320xf32>, %arg1: tensor<32768xf32>, %arg2: tensor<11520xf32>, %arg3: tensor<64xf32> {rock.prefill = 0.000000e+00 : f32}, %arg4: tensor<64xf32> {rock.prefill = 0.000000e+00 : f32}, %arg5: tensor<2621440xf32> {rock.prefill = 0.000000e+00 : f32}) -> (tensor<64xf32>, tensor<64xf32>, tensor<2621440xf32>) attributes {rock.arch = "gfx942:sramecc+:xnack-", rock.enable_splitk_for_tuning, rock.kernel = "mixr"} {
    ...
    %conv = rock.conv(%3, %2) {dilations = [1 : index, 1 : index], filter_layout = ["g", "k", "c", "y", "x"], input_layout = ["ni", "gi", "ci", "hi", "wi"], output_layout = ["no", "go", "ko", "ho", "wo"], padding = [1 : index, 1 : index, 1 : index, 1 : index], strides = [1 : index, 1 : index]} : tensor<1x320x4x3x3xf32>, tensor<2x1x4x64x64xf32> -> tensor<2x1x320x64x64xf32>
    ...
    %fused_flat = rock.transform %fused_add by <affine_map<(d0) -> (d0 floordiv 1310720, (d0 floordiv 40960) mod 32, (d0 floordiv 4096) mod 10, (d0 floordiv 64) mod 64, d0 mod 64)> by [<Merge{2, 32, 10, 64, 64} ["dim0"] at [0] -> ["col0", "col1", "col2", "col3", "col4"] at [0, 1, 2, 3, 4]>] bounds = [2621440] -> [2, 32, 10, 64, 64]> : tensor<2x32x10x64x64xf32> to tensor<2621440xf32>
    %scaled = arith.mulf %fused_add, %cst : tensor<2x32x10x64x64xf32>
    %scaled_flat = rock.transform %scaled by <affine_map<(d0, d1, d2) -> (d0, d1, d2 floordiv 4096, (d2 floordiv 64) mod 64, d2 mod 64)> by [<PassThrough ["dim0"] at [0] -> ["dim0"] at [0]>, <PassThrough ["dim1"] at [1] -> ["dim1"] at [1]>, <Merge{10, 64, 64} ["dim2"] at [2] -> ["col2", "col3", "col4"] at [2, 3, 4]>] bounds = [2, 32, 40960] -> [2, 32, 10, 64, 64]> : tensor<2x32x10x64x64xf32> to tensor<2x32x40960xf32>
    %reduced_0 = rock.reduce sum %scaled_flat {axis = 2 : index} : tensor<2x32x40960xf32> -> tensor<2x32x1xf32>
    %reduced_0_flat = rock.transform %reduced_0 by <affine_map<(d0) -> (d0 floordiv 32, d0 mod 32, 0)> by [<Merge{2, 32, 1} ["dim0"] at [0] -> ["col0", "col1", "col2"] at [0, 1, 2]>] bounds = [64] -> [2, 32, 1]> : tensor<2x32x1xf32> to tensor<64xf32>
    ...
    %squared_flat = rock.transform %squared_scaled by <affine_map<(d0, d1, d2) -> (d0, d1, d2 floordiv 4096, (d2 floordiv 64) mod 64, d2 mod 64)> by [<PassThrough ["dim0"] at [0] -> ["dim0"] at [0]>, <PassThrough ["dim1"] at [1] -> ["dim1"] at [1]>, <Merge{10, 64, 64} ["dim2"] at [2] -> ["col2", "col3", "col4"] at [2, 3, 4]>] bounds = [2, 32, 40960] -> [2, 32, 10, 64, 64]> : tensor<2x32x10x64x64xf32> to tensor<2x32x40960xf32>
    %reduced_1 = rock.reduce sum %squared_flat {axis = 2 : index} : tensor<2x32x40960xf32> -> tensor<2x32x1xf32>
    %reduced_1_flat = rock.transform %reduced_1 by <affine_map<(d0) -> (d0 floordiv 32, d0 mod 32, 0)> by [<Merge{2, 32, 1} ["dim0"] at [0] -> ["col0", "col1", "col2"] at [0, 1, 2]>] bounds = [64] -> [2, 32, 1]> : tensor<2x32x1xf32> to tensor<64xf32>
    %out3 = rock.store %reduced_0_flat to %arg3 by set : tensor<64xf32> -> tensor<64xf32> to tensor<64xf32>
    %out4 = rock.store %reduced_1_flat to %arg4 by set : tensor<64xf32> -> tensor<64xf32> to tensor<64xf32>
    %out5 = rock.store %fused_flat to %arg5 by set : tensor<2621440xf32> -> tensor<2621440xf32> to tensor<2621440xf32>
