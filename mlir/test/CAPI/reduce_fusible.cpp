  ...
  const char *mlirModuleText = R"mlir(
    module {
      func.func @mlir_convolution_reshape_reshape_broadcast_add_mul_reshape_reduce_sum_reshape_mul_mul_reshape_reduce_sum_reshape(%arg0: tensor<32768xf32>, %arg1: tensor<11520xf32>, %arg2: tensor<320xf32>, %arg3: tensor<64xf32> {rock.prefill = 0.000000e+00 : f32}, %arg4: tensor<64xf32> {rock.prefill = 0.000000e+00 : f32}, %arg5: tensor<2621440xf32>) -> (tensor<64xf32>, tensor<64xf32>, tensor<2621440xf32>) attributes {rock.arch = "gfx942:sramecc+:xnack-", rock.enable_splitk_for_tuning, rock.kernel = "mixr", rock.num_cu = 304 : i64, rock.num_chiplets = 8 : i64} {
        ...
        %10 = rock.transform %fused_add by <affine_map<(d0) -> (d0 floordiv 1310720, (d0 floordiv 40960) mod 32, (d0 floordiv 4096) mod 10, (d0 floordiv 64) mod 64, d0 mod 64)> by [<Merge{2, 32, 10, 64, 64} ["dim0"] at [0] -> ["col0", "col1", "col2", "col3", "col4"] at [0, 1, 2, 3, 4]>] bounds = [2621440] -> [2, 32, 10, 64, 64]> : tensor<2x32x10x64x64xf32> to tensor<2621440xf32>
        %scaled = arith.mulf %fused_add, %cst : tensor<2x32x10x64x64xf32>
        %11 = rock.transform %scaled by <affine_map<(d0, d1, d2) -> (d0, d1, d2 floordiv 4096, (d2 floordiv 64) mod 64, d2 mod 64)> by [<PassThrough ["dim0"] at [0] -> ["dim0"] at [0]>, <PassThrough ["dim1"] at [1] -> ["dim1"] at [1]>, <Merge{10, 64, 64} ["dim2"] at [2] -> ["col2", "col3", "col4"] at [2, 3, 4]>] bounds = [2, 32, 40960] -> [2, 32, 10, 64, 64]> : tensor<2x32x10x64x64xf32> to tensor<2x32x40960xf32>
        ...
        %13 = rock.transform %squared_scaled by <affine_map<(d0, d1, d2) -> (d0, d1, d2 floordiv 4096, (d2 floordiv 64) mod 64, d2 mod 64)> by [<PassThrough ["dim0"] at [0] -> ["dim0"] at [0]>, <PassThrough ["dim1"] at [1] -> ["dim1"] at [1]>, <Merge{10, 64, 64} ["dim2"] at [2] -> ["col2", "col3", "col4"] at [2, 3, 4]>] bounds = [2, 32, 40960] -> [2, 32, 10, 64, 64]> : tensor<2x32x10x64x64xf32> to tensor<2x32x40960xf32>
