...
# (label, arch, perf_config, expected). perf_config follows the 12-field attn:v2
# tuple parsed by GemmGemmParamsAttr: (mPerBlock, nPerBlock, nPerBlockG1,
# kPerBlock, kpack, numCTAs, numWaves, matrixInstrNonkdim, splitKFactor,
# numStages, wavesPerEU, gridGroupSize). nPerBlockG1 (the 3rd field) doesn't
# affect this predicate, so it's pinned to 0 (untiled) throughout.
...
CASES = [
    ...
    ("cfg1 bf16", GFX950_HIP, (256, 32, 0, 16, 1, 1, 1, 0, 1, 2, 8, 1), False),
    # wpe=0 -> eff_wpe=1, budget=512. (256*128)/64 = 512 fits exactly -> ACCEPT.
    ("cfg2 i8", GFX950_HIP, (256, 128, 0, 64, 1, 1, 1, 16, 1, 1, 0, 8), True),
    ...
    ("gfx942 fits", "gfx942", (64, 64, 0, 16, 1, 1, 1, 16, 1, 1, 8, 0), True),
    ("gfx942 overflow", "gfx942", (128, 64, 0, 16, 1, 1, 1, 16, 1, 1, 8, 0), False),
    ...
    ("gfx1030 fits", "gfx1030", (64, 32, 0, 16, 1, 1, 1, 16, 1, 1, 8, 0), True),
    ("gfx1030 boundary", "gfx1030", (64, 64, 0, 16, 1, 1, 1, 16, 1, 1, 8, 0), True),

    # gfx1100 (RDNA3, wave=32, vgprs=1536): wpe=8 budget=192.
    # (64*64)/32 = 128 -> ACCEPT; (128*64)/32 = 256 -> REJECT.
    ("gfx1100 fits", "gfx1100", (64, 64, 0, 16, 1, 1, 1, 16, 1, 1, 8, 0), True),
    ("gfx1100 overflow", "gfx1100", (128, 64, 0, 16, 1, 1, 1, 16, 1, 1, 8, 0), False),
    ...
    ("wpe=0 1024-tile gfx950", GFX950_HIP, (256, 256, 0, 16, 1, 1, 1, 16, 1, 1, 0, 0), False),
    # (256*256)/32 = 2048 > 1536 -> REJECT (gfx1100, doubled file).
    ("wpe=0 2048-tile gfx1100", "gfx1100", (256, 256, 0, 16, 1, 1, 1, 16, 1, 1, 0, 0), False),
    # Companion ACCEPTs: small tile, and bumping num_waves -> small accum/thread.
    ("wpe=0 small gfx950", GFX950_HIP, (64, 64, 0, 16, 1, 1, 1, 16, 1, 1, 0, 0), True),
    # (256*256)/(8*64) = 128 <= 512 -> ACCEPT.
    ("wpe=0 multi-wave gfx950", GFX950_HIP, (256, 256, 0, 16, 1, 1, 8, 16, 1, 1, 0, 0), True),

    # Multi-wave: num_waves=4 quadruples threads -> accum/thread / 4.
    # (128*128)/(4*64) = 64 fits gfx950 wpe=8 budget=64.
    ("4-wave fits", GFX950_HIP, (128, 128, 0, 16, 1, 1, 4, 16, 1, 1, 8, 0), True),
]
