...
class CommandAndResultTest(unittest.TestCase):
    ...
    def test_command_construction(self):
        ...
        self.assertEqual(Path(tune[1]).parent, runner.BENCHMARK_TOOLS_DIR)
        self.assertNotEqual(Path(tune[1]).parent, branch.source)
        ...
        benchmark = runner.benchmark_command(branch, "-t f16 -g 1", "-current_seq_len=3",
                                             Path("/tmp.csv"))
        self.assertEqual(Path(benchmark[1]).parent, runner.BENCHMARK_TOOLS_DIR)
