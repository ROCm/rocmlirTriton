// RUN: rocmlir-gen --arch gfx90a --store-method atomic_add --operation gemm -t f16 -p | AMDGCN_ENABLE_DUMP=1 rocmlir-driver -c 2>&1 | FileCheck %s --check-prefix=GLOBAL_ATOMIC_CMPSWAP
...
// RUN: rocmlir-gen --arch gfx90a --store-method atomic_add --operation gemm -t f32 -p | AMDGCN_ENABLE_DUMP=1 rocmlir-driver -c 2>&1 | FileCheck %s --check-prefix=GLOBAL_ATOMIC_CMPSWAP
...
// XXX: rocmlir-gen --arch gfx1100 --store-method atomic_add --operation gemm -t f32 -p | AMDGCN_ENABLE_DUMP=1 rocmlir-driver -c 2>&1 | FileCheck %s --check-prefix=BUFFER_ATOMIC_ADD_F32
...
// XXX: rocmlir-gen --arch gfx1201 --store-method atomic_add --operation gemm -t f32 -p | AMDGCN_ENABLE_DUMP=1 rocmlir-driver -c 2>&1 | FileCheck %s --check-prefix=BUFFER_ATOMIC_ADD_F32
