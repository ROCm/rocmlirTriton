...
// pipeline.
//
// The AMDGPU InstCombine erases raw.ptr.buffer.store intrinsics whose offset is
// provably past the buffer descriptor's NumRecords. This GEMM has M = 77
// against mPerBlock = 128, so the masked tail leaves whole 16-row accumulator
// tiles with no surviving store and the wmma chains feeding them become dead.
// Those chains are self-referential across the K loop, each iteration's
// accumulator being a phi of the previous one, so plain DCE cannot reclaim them
// and the default pipeline has no ADCE run left after the folds happen.
//
