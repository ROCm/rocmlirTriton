...
//
// Three things are pinned here. On gfx1250 tanh must reach the native v_tanh_f32
...
// shows up nowhere but the ISA, hence this test.
//
// The third is the OCML call itself, under --disable-fast-math, where no type
// takes an expansion at all. That one is about the symbol resolving rather than
// about instruction selection: the call goes out to a name this repo spells by
// hand, and nothing before the binary stage can tell whether it names something
// that exists.
//
...
// The OCML path, taken by every type once fast math is off. One arch is enough:
// what this covers is `__ocml_tanh_f32` naming something that the `ocml.bc`
// TritonToHsaco links actually defines, and that link is the same everywhere.
// Reaching the binary stage is most of the check, since an unresolved reference
// fails there rather than producing a kernel.
// RUN: rocmlir-gen --clone-harness -arch gfx942 -fut mlir_convolution_tanh %s \
// RUN: | rocmlir-driver -disable-fast-math -arch=gfx942 -kernel-pipeline=migraphx,highlevel -host-pipeline=migraphx,highlevel \
// RUN: | env AMDGCN_ENABLE_DUMP=1 rocmlir-driver -disable-fast-math -arch=gfx942 -kernel-pipeline=gpu,triton,binary -o /dev/null > %t.ocml 2>&1
// RUN: FileCheck %s --check-prefix=OCML < %t.ocml
// RUN: FileCheck /dev/null --implicit-check-not=s_swappc < %t.ocml
...
// The library body is inlined, so the resolved call leaves the block label the
// inliner names after it rather than a call instruction. The implicit-check-not
// on the RUN is the other half: no call survives to be resolved at run time.
//
// OCML computes the same identity, so its exp and reciprocal are the same two
// instructions the expansion above emits and neither one tells the paths apart.
// What only the library body has is the machinery around them: a compensated
// argument reduction feeding the exponential, and a branch to a small-|x|
// polynomial. Those are the ~25 instructions the approximation exists to avoid.
// OCML-DAG: __ocml_tanh_f32.exit
// OCML-DAG: v_rndne_f32
// OCML-DAG: v_ldexp_f32
// OCML-DAG: s_cbranch_execz
