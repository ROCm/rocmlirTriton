...
// is built the way it is. What this file adds is the bf8 mnemonics, which are a
// separate set of instructions from the fp8 ones.
//
// This is a separate file rather than a second kernel in fp8_cvt_lowering.mlir
// because the Triton half of the kernel pipeline is parameterized per module
...
// RUN: rocmlir-gen --clone-harness -arch gfx942 -fut mlir_bf8_cvt %s \
...
// RUN: rocmlir-gen --clone-harness -arch gfx950 -fut mlir_bf8_cvt %s \
...
// RUN: rocmlir-gen --clone-harness -arch gfx1250 -fut mlir_bf8_cvt %s \
...
module {
  // Quantized GEMM over f8E5M2 with a bf8 residual added in the epilogue.
