...
// whose GEMM M dimension is 1 but padded up to mPerBlock, it keeps the whole
// padded-tile epilogue alive.
//
// rock-fold-oob-buffer-ops erases those stores, after which the matrix multiplies
// and the sigmoid's exp2 that only fed padding rows die with them. That reclaim
// is what this test adds over the counters pinned in the split-soffset test.
//
// perf_config is pinned so the counts do not move with the tuning heuristics;
// they differ per target because the tile shapes and matrix instructions do.
// Without the fold gfx1100 emits 32 stores, 16 matrix multiplies and 32 exp2
// calls instead of 2, 8 and 2; the other targets roughly double.
