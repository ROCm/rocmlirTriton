...
// Flash-decoding split-KV promotion with a KV-cache currentSeqLen *and* a
// prefix-causal prefixOffset. When the pass promotes splitKV it splits the
// batch dimension; both optional i32 tensors (currentSeqLen and prefixOffset)
// must be sliced the same way Q/K/V are (fixing the split-KV coordinate to 0),
// otherwise the sliced attention op fails batch-size verification. Here batch
// 24 = 12 heads * 2 split-KV, so after the split both currentSeqLen and
// prefixOffset go from tensor<24xi32> to tensor<12xi32>.
...
module {
  // CHECK-LABEL: func.func @mlir_flash_decode_kvcache_prefix_causal
  // CHECK: rock.attention{
  // CHECK-NEXT: qk = %{{.*}} * %{{.*}} : tensor<12x256x256xf16>, tensor<12x256x256xf16>
  // Both the KV-cache currentSeqLen and the prefix-causal prefixOffset are
  // sliced from tensor<24xi32> to tensor<12xi32>.
  // CHECK-NEXT: currentSeqLen = (%{{.*}} : tensor<12xi32>)
  // CHECK-NEXT: prefixOffset = (%{{.*}} : tensor<12xi32>)
  // CHECK: splitKV = 2 : i32
  func.func @mlir_flash_decode_kvcache_prefix_causal(%arg0: tensor<786432xf16>, %arg1: tensor<786432xf16>, %arg2: tensor<786432xf16>, %csl: tensor<12xi32>, %poff: tensor<12xi32>) -> (tensor<24x256x256xf16>, tensor<24x256xf32>) attributes {rock.arch = "amdgcn-amd-amdhsa:gfx1100", rock.kernel = "mixr", rock.num_cu = 48 : i64} {
    ...
    %p0 = rock.transform %poff by #cslt0 : tensor<12xi32> to tensor<1x12x1xi32>
    %p1 = rock.transform %p0 by #cslt1 : tensor<1x12x1xi32> to tensor<1x12x2xi32>
    %p2 = rock.transform %p1 by #cslt2 : tensor<1x12x2xi32> to tensor<24xi32>
    %result, %lseOut = rock.attention{
     qk = %6 * %8 : tensor<24x256x256xf16>, tensor<24x256x128xf16>
     currentSeqLen = (%c2 : tensor<24xi32>)
     prefixOffset = (%p2 : tensor<24xi32>)
     softmax(qk) * %9 : tensor<24x128x256xf16>
    } {causal, numHeadsKV = 1 : i32, numHeadsQ = 1 : i32, perf_config = "attn:v1:64,64,128,1,1,4,0,1,1,0,0", softmaxType = f32, splitKV = 1 : i32} -> tensor<24x256x256xf16>, tensor<24x256xf32>
