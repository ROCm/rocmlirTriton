...
module {
  func.func @mlir_unsqueeze_transpose_reshape_reshape_transpose_dot_convert_reshape_reduce_max_reshape_sub_exp_reshape_reduce_sum_reshape_div_convert_dot_log_add(%arg0: tensor<786432xf16>, %arg1: tensor<786432xf16>, %arg2: tensor<786432xf16>) -> (tensor<1572864xf16>, tensor<6144xf32>) attributes {rock.arch = "amdgcn-amd-amdhsa:gfx1100", rock.kernel = "mixr", rock.num_cu = 48 : i64} {
    ...
    %10 = tensor.empty() : tensor<24x256x256xf16>
    %11 = tensor.empty() : tensor<24x256xf32>

    // CHECK: rock.attention{
    // CHECK-NEXT: qk = %{{.*}} * %{{.*}} : tensor<12x256x256xf16>, tensor<12x256x256xf16>
    // CHECK: softmax(qk) * %{{.*}} : tensor<12x256x256xf16>
    // CHECK: perf_config = "attn:v1:64,64,128,1,1,4,0,1,1,0,0"
    // CHECK: splitKV = 2
    ...
    } {firstGemmIndices = array<i64: 0>, numHeadsKV = 1 : i32, numHeadsQ = 1 : i32, perf_config = "attn:v1:64,64,128,1,1,4,0,1,1,0,0", softmaxType = f32, splitKV = 1 : i32} -> tensor<24x256x256xf16>, tensor<24x256xf32>
