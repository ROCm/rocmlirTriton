...
//
// Same flash decoding graph as detect-flash-decoding.mlir, except that K has no
// leading unit dimension, so its flatten is Merge{12, 256, 256}. That Merge
// does not produce the batch dimension of the attention operand and must not
// be read as splitKV = 256.

// RUN: rocmlir-opt --rock-detect-flash-decoding %s | FileCheck %s

#map = affine_map<(d0, d1, d2) -> ((d0 * 256 + d1) * 256 + d2)>
...
#map7 = affine_map<(d0) -> (d0 floordiv 65536, (d0 floordiv 256) mod 256, d0 mod 256)>
...
#transform_map = #rock.transform_map<#map by [<Unmerge{12, 256, 256} ["exp0", "exp1", "exp2"] at [0, 1, 2] -> ["dim0"] at [0]>] bounds = [12, 256, 256] -> [786432]>
...
#transform_map3 = #rock.transform_map<#map3 by [<PassThrough ["dim0", "dim2", "dim1"] at [0, 1, 2] -> ["dim0", "dim2", "dim1"] at [0, 2, 1]>] bounds = [12, 256, 256] -> [12, 256, 256]>
...
#transform_map7 = #rock.transform_map<#map7 by [<Merge{12, 256, 256} ["dim0"] at [0] -> ["col0", "col1", "col2"] at [0, 1, 2]>] bounds = [786432] -> [12, 256, 256]>
...
#transform_map13 = #rock.transform_map<#map7 by [<Merge{24, 256, 256} ["dim0"] at [0] -> ["col0", "col1", "col2"] at [0, 1, 2]>] bounds = [1572864] -> [24, 256, 256]>
module {
  func.func @k_flatten_merge(%arg0: tensor<786432xf16>, %arg1: tensor<786432xf16>, %arg2: tensor<786432xf16>) -> (tensor<1572864xf16>, tensor<6144xf32>) attributes {rock.arch = "amdgcn-amd-amdhsa:gfx942", rock.kernel = "mixr"} {
    %0 = rock.transform %arg1 by #transform_map : tensor<786432xf16> to tensor<12x256x256xf16>
    ...
    %3 = rock.transform %0 by #transform_map3 : tensor<12x256x256xf16> to tensor<12x256x256xf16>
    ...
    %7 = rock.transform %3 by #transform_map7 : tensor<12x256x256xf16> to tensor<786432xf16>
    ...
    // CHECK: softmax(qk) * %{{.*}} : tensor<12x256x256xf16>
    // CHECK: splitKV = 2
