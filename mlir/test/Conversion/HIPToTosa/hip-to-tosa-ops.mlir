...
// CHECK: tosa.matmul %arg1, %arg2, %{{.*}}, %{{.*}} {acc_type = f32}
...
// CHECK: tosa.reshape %arg1, %{{.*}} : (tensor<4x4x256x16xf32>, !tosa.shape<3>) -> tensor<16x256x16xf32>
// CHECK: tosa.reshape %arg2, %{{.*}} : (tensor<4x4x16x256xf32>, !tosa.shape<3>) -> tensor<16x16x256xf32>
...
// transA/transB are not handled yet, so the op must survive untouched.
