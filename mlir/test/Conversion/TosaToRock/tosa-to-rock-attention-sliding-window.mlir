...
// The KV-cache clip (max/min on currentSeqLen) is re-emitted in front of the
// attention op, and the sliding-window size is captured as the slidingWindowSize
// attribute so the gridwise lowering can mask keys below
// max(0, currentSeqLen - slidingWindowSize).
