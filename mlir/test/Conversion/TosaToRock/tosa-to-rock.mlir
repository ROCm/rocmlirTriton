...
// DISABLED-CHECK-LABEL: mlir_dot_transpose_add
// DISABLED-CHECK: %[[gemmRes:.*]] = rock.gemm %{{.*}} = %{{.*}} * %{{.*}} storeMethod =  set : tensor<1x4x5xf32> = tensor<1x4x5xf32> * tensor<1x5x5xf32> -> tensor<1x4x5xf32>
// DISABLED-CHECK-NEXT: %{{.*}} = tosa.reshape %[[gemmRes]]
// DISABLED-CHECK-NEXT: %[[transRes:.*]] = rock.transform %[[gemmRes]] by #{{.*}} : tensor<1x4x5xf32> to tensor<1x5x4xf32>
...
// DISABLED-CHECK-LABEL: mlir_scaled_gemm_both_scales
// DISABLED-CHECK: rock.gemm %{{.*}} = %{{.*}} scaled by %{{.*}} * %{{.*}} scaled by %{{.*}}
...
// TODO(roctriton): Scaled gemm tests need rework
// DISABLED-CHECK-LABEL: mlir_scaled_gemm_batched
// DISABLED-CHECK: rock.gemm %{{.*}} = %{{.*}} scaled by %{{.*}} * %{{.*}} scaled by %{{.*}}
...
// TODO(roctriton): Scaled gemm tests need rework
// DISABLED-CHECK-LABEL: mlir_scaled_gemm_with_transpose_a
// DISABLED-CHECK: rock.transform
// DISABLED-CHECK: rock.gemm %{{.*}} = %{{.*}} scaled by %{{.*}} * %{{.*}} scaled by %{{.*}}
...
// TODO(roctriton): Scaled gemm tests need rework
// DISBLED-CHECK-LABEL: mlir_scaled_gemm_with_transpose_b
// DISBLED-CHECK: rock.transform
// DISBLED-CHECK: rock.gemm %{{.*}} = %{{.*}} scaled by %{{.*}} * %{{.*}} scaled by %{{.*}}
...
// TODO(roctriton): Scaled gemm tests need rework
// DISABLED-CHECK-LABEL: mlir_scaled_gemm_with_a_broadcast
// DISABLED-CHECK-DAG: %[[A_BROADCAST:.*]] = rock.transform %{{.*}} by #{{.*}} : tensor<1x1x512xf4E2M1FN> to tensor<1x16x512xf4E2M1FN>
// DISABLED-CHECK-DAG: %[[A_SCALE_BROADCAST:.*]] = rock.transform %{{.*}} by #{{.*}} : tensor<1x1x512xf8E8M0FNU> to tensor<1x16x512xf8E8M0FNU>
// DISABLED-CHECK: rock.gemm %{{.*}} = %[[A_BROADCAST]] scaled by %[[A_SCALE_BROADCAST]] * %{{.*}} scaled by %{{.*}}
...
// TODO(roctriton): Scaled gemm tests need rework
// DISABLED-CHECK-LABEL: mlir_quant_dot_fp4_fp32_scales
// DISABLED-CHECK-DAG: rock.transform %{{.*}} by #{{.*}} : tensor<196608xf4E2M1FN> to tensor<1x256x768xf4E2M1FN>
// DISABLED-CHECK-DAG: rock.transform %{{.*}} by #{{.*}} : tensor<38608896xf4E2M1FN> to tensor<1x768x50272xf4E2M1FN>
// DISABLED-CHECK-DAG: rock.transform %{{.*}} by #{{.*}} : tensor<6144xf32> to tensor<1x8x1x768xf32>
// DISABLED-CHECK-DAG: rock.transform %{{.*}} by #{{.*}} : tensor<1206528xf32> to tensor<1x768x1571x1xf32>
// DISABLED-CHECK-DAG: %[[A_SCALE:.*]] = tosa.cast %{{.*}} : (tensor<1x256x768xf32>) -> tensor<1x256x768xf8E8M0FNU>
// DISABLED-CHECK-DAG: %[[B_SCALE:.*]] = tosa.cast %{{.*}} : (tensor<1x768x50272xf32>) -> tensor<1x768x50272xf8E8M0FNU>
// DISABLED-CHECK: rock.gemm %{{.*}} = %{{.*}} scaled by %{{.*}} * %{{.*}} scaled by %{{.*}} storeMethod =  set
