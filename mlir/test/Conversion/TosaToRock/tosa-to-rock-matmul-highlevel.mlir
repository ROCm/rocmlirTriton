...
module {
  ...
  func.func private @dot_tr_collapse_reshape2(%arg0: tensor<2x320x64x64xf32>, %arg1: tensor<1x320x320xf32>) -> tensor<2x4096x320xf32> attributes {rock.arch = "##TOKEN_ARCH##", rock.kernel} {
    ...
    // CHECK-DAG: %[[TRANSFORM_ARG0_1:.*]] = rock.transform %[[TRANSFORM_ARG0_0]] by {{.*}} : tensor<2x64x64x320xf32> to tensor<2x4096x320xf32>
    // CHECK-DAG: %[[TRANSFORM_ARG0_2:.*]] = rock.transform %[[TRANSFORM_ARG0_1]] by {{.*}} : tensor<2x4096x320xf32> to tensor<8192x320xf32>
    // CHECK-DAG: %[[TRANSFORM_ARG1_1:.*]] = rock.transform %[[TRANSFORM_ARG1_0]] by {{.*}} : tensor<2x320x320xf32> to tensor<320x320xf32>
