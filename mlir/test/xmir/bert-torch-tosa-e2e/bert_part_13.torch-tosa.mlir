// RUN: rocmlir-gen -fut bert_part_13 -arch %arch --clone-harness %s | rocmlir-driver -host-pipeline highlevel -kernel-pipeline highlevel -arch %arch | rocmlir-gen -ph -print-results -rand 1 -rand_type float -fut bert_part_13 --verifier clone - | rocmlir-driver -kernel-pipeline full | mlir-runner --shared-libs=%linalg_test_lib_dir/%shlibprefixmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/%shlibprefixconv-validation-wrappers%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_runner_utils%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_float16_utils%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_c_runner_utils%shlibext,%linalg_test_lib_dir/%shlibprefixmlir_async_runtime%shlibext --entry-point-result=void | FileCheck %s
// CHECK: [1 1 1]
module {
  func.func @bert_part_13(%arg0: tensor<1x12x12x32xf32>, %arg1: tensor<384x384xf32>, %arg2: tensor<1x1x384xf32>, %arg3: tensor<1x12x384xf32>) -> (tensor<1x12x384xf32>) {
