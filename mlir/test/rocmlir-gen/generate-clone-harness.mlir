...
// CHECK-LABEL: module
// CHECK: func.func private @mlir_reshape_convolution_cpu_host(%{{.*}}: tensor<256xf32>, %{{.*}}: tensor<9xf32>) -> tensor<1024xf32>
// CHECK: func.func private @mlir_reshape_convolution(%{{.*}}: tensor<256xf32>, %{{.*}}: tensor<9xf32>, %{{.*}}: tensor<1024xf32>) -> tensor<1024xf32> attributes {rock.kernel}
// CHECK-LABEL: func.func @main()
// CHECK: call @mlir_reshape_convolution_gpu({{.*}}) : (memref<256xf32>, memref<9xf32>, memref<1024xf32>) -> ()
// CHECK: call @mlir_reshape_convolution_cpu_host({{.*}}) : (tensor<256xf32>, tensor<9xf32>) -> tensor<1024xf32>
// CHECK: call @mlir_reshape_convolution_verify{{[0-9]+}}({{.*}}) : (memref<1024xf32>, memref<1024xf32>) -> ()
// CHECK: func.func @mlir_reshape_convolution_gpu(%{{.*}}: memref<256xf32>, %{{.*}}: memref<9xf32>, %{{.*}}: memref<1024xf32>)

// CHECK_FULL-LABEL: module
// CHECK_FULL: llvm.func @mlir_reshape_convolution_gpu
