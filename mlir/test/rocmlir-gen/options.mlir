...
// --cpu-timers requires a host harness; without -ph the driver must exit.
// RUN: not rocmlir-gen --arch %arch --operation gemm -t f32 -g 1 -m 64 -k 64 -n 64 --cpu-timers 2>&1 | FileCheck %s --check-prefix=ERR_CPU_TIMERS
// ERR_CPU_TIMERS: --cpu-timers requires host harness generation
...
// A negative sliding_window_look_back is a user error: the flag's contract is
// "positive integer, 0 disables", so it must be rejected instead of silently
// disabling sliding-window masking.
// RUN: not rocmlir-gen --arch %arch --operation attention -t f16 -seq_len_q 256 -seq_len_k 256 -head_dim_qk 32 -head_dim_v 32 -sliding_window_look_back=-16 2>&1 | FileCheck %s --check-prefix=ERR_SLIDING_WINDOW_NEG
// ERR_SLIDING_WINDOW_NEG: sliding_window_look_back must be non-negative
...
// RUN: not rocmlir-gen --arch %arch --operation attention -t f16 -seq_len_q 1 -seq_len_k 64 -head_dim_qk 32 -head_dim_v 32 -last_valid_kv_index=32 -sliding_window_look_back=128 2>&1 | FileCheck %s --check-prefix=ERR_SLIDING_WINDOW_TOO_LARGE
// ERR_SLIDING_WINDOW_TOO_LARGE: sliding_window_look_back must not exceed seq_len_k
