...
//
// RUN: rocmlir-gen --arch gfx90a:sramecc+:xnack- --operation attention -last_valid_kv_index=33 -return_lse -split_kv 8 -num_heads_q 4 -num_heads_kv 2 -seq_len_q 1 -seq_len_k 1024 -head_dim_qk 32 -head_dim_v 32 -t f16 -pv | rocmlir-opt | FileCheck %s --enable-var-scope
// RUN: rocmlir-gen --arch gfx90a:sramecc+:xnack- --operation attention -last_valid_kv_index=33 -return_lse -split_kv 8 -num_heads_q 4 -num_heads_kv 2 -seq_len_q 1 -seq_len_k 1024 -head_dim_qk 32 -head_dim_v 32 -t bf16 -pv | rocmlir-opt | FileCheck %s --check-prefix=BF16
...
// CHECK: %[[safeMx:.+]] = tosa.maximum %[[mx]], %{{.+}} : (tensor<4x1x1x1xf32>, tensor<4x1x1x1xf32>) -> tensor<4x1x1x1xf32>
...
// CHECK: %[[safeSum:.+]] = tosa.maximum %[[sum]], %{{.+}} : (tensor<4x1x1x1xf32>, tensor<4x1x1x1xf32>) -> tensor<4x1x1x1xf32>
...
// BF16-LABEL: func.func @rock_attention_gpu
// BF16: tosa.reshape %{{.+}} : (tensor<1024xbf16>, !tosa.shape<4>) -> tensor<4x8x1x32xbf16>
// BF16: tosa.cast %{{.+}} : (tensor<4x8x1x32xbf16>) -> tensor<4x8x1x32xf32>
// BF16: tosa.reduce_max %{{.+}} {axis = 1 : i32} : (tensor<4x8x1x1xf32>) -> tensor<4x1x1x1xf32>
// BF16: tosa.cast %{{.+}} : (tensor<4x1x1x32xf32>) -> tensor<4x1x1x32xbf16>
