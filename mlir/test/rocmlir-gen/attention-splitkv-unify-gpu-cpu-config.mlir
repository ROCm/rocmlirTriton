...
//
// With head_dim_v=208 on gfx942 the entries preceding the current
// nPerBlockG0=64 choice are inapplicable to this shape. Both paths must skip
// them identically and select the same first applicable config.

// CPU side: the reference split-KV partition is derived from gemm0's
// nPerBlockG0. nPerBlockG0=64 -> valid split mask [4, 1, 5, 2, 5].
...
// (params0) must have the same nPerBlock=64 that the CPU mask above assumes.
...
// CPU-CONFIG: "tosa.const"() <{values = dense<{{\[+}}4{{\]+}}, {{\[+}}1{{\]+}}, {{\[+}}5{{\]+}}, {{\[+}}2{{\]+}}, {{\[+}}5{{\]+}}> : tensor<5x1x1x1xi32>}>

// GPU-CONFIG-DAG: [[GEMM0:#gemm_params[0-9]*]] = #rock.gemm_params<mPerBlock = {{[0-9]+}}, nPerBlock = 64,
