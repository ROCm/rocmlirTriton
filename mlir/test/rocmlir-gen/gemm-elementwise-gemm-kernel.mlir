// The attribute list is checked exactly, so num_cu is pinned rather than left
// to the default: an unpinned run picks the count up from a device whose
// architecture matches --arch, and the attribute would then appear only on
// some machines. num-cu-default.mlir covers the defaulting itself.
