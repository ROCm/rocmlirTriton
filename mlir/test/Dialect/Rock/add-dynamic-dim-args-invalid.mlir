...
// more than one gemm-like op has to be rejected rather than numbered
// ambiguously.
...
// expected-error @+1 {{dynamic shapes are only supported for kernels with exactly one rock.gemm or rock.attention, found 2}}
...
// A kernel with no gemm-like op at all, such as a pure elementwise fusion, is
// equally outside what the positional convention can describe.

// expected-error @+1 {{dynamic shapes are only supported for kernels with exactly one rock.gemm or rock.attention, found 0}}
