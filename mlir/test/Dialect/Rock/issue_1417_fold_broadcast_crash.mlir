...
// CHECK: Broadcast{1}
func.func @mlir_reshape_reshape_transpose_dot(%arg0: tensor<1x256x32x64xf32>, %arg1: tensor<1x4x256x64xf32>) -> tensor<1x32x256x256xf32> attributes {rock.arch = "gfx908:sramecc+:xnack-", rock.kernel} {
  ...
  %6 = rock.gemm %5 * tr %4 : tensor<32x256x64xf32> * tensor<32x256x64xf32> -> tensor<32x256x256xf32>
  %7 = rock.transform %6 by <affine_map<(d0, d1, d2, d3) -> (d0 * 32 + d1, d2, d3)> by [<Unmerge{1, 32} ["exp0", "exp1"] at [0, 1] -> ["dim0"] at [0]>, <PassThrough ["dim1"] at [2] -> ["dim1"] at [1]>, <PassThrough ["dim2"] at [3] -> ["dim2"] at [2]>] bounds = [1, 32, 256, 256] -> [32, 256, 256]> : tensor<32x256x256xf32> to tensor<1x32x256x256xf32>
  return %7 : tensor<1x32x256x256xf32>
}
