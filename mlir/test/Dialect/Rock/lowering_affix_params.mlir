...
// CHECK: module {{.*}}
// CHECK-NEXT: func.func @{{.*}}(%{{.*}}: tensor<{{.*}}>, %{{.*}}: tensor<{{.*}}>, %arg2: tensor<{{.*}}>) -> tensor<{{.*}}> attributes {rock.arch = {{.*}}, rock.block_size = {{.*}} : i32, rock.enable_splitk_for_tuning, rock.kernel, rock.num_chiplets = {{.*}} : i64, rock.num_cu = {{.*}} : i32}
