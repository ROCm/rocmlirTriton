...
//   arith.divf                -> nsz + arcp     (nsz peepholes + hw reciprocal)
//   arith.{add,sub,mul}f      -> nsz + contract (nsz peepholes + fma fusion)
//   arith.negf                -> nsz            (sign-bit XOR peephole)
//   math.* transcendental     -> afn            (hardware approximate impl)
...
module {
  ...
  // CHECK: arith.divf %{{.*}}, %{{.*}} fastmath<nsz,arcp> : f32
  ...
  // CHECK: arith.divf %{{.*}}, %{{.*}} fastmath<nsz,arcp> : tensor<2x3xf32>
  ...
  // CHECK: arith.divf %{{.*}}, %{{.*}} fastmath<nnan,nsz,arcp> : f32
  ...
  // CHECK: math.log %{{.*}} fastmath<afn> : f32
  // CHECK: math.sqrt %{{.*}} fastmath<afn> : f32
  // CHECK: math.rsqrt %{{.*}} fastmath<afn> : f32
  // CHECK: math.sin %{{.*}} fastmath<afn> : f32
  // CHECK: math.tanh %{{.*}} fastmath<afn> : f32
  ...
  // FAST-NEXT: math.exp %{{.*}} fastmath<afn> : tensor<2x3xf32>
  // FAST-NEXT: arith.addf %{{.*}}, %{{.*}} fastmath<nsz,contract> : tensor<2x3xf32>
  // FAST-NEXT: arith.divf %{{.*}}, %{{.*}} fastmath<nsz,arcp> : tensor<2x3xf32>
