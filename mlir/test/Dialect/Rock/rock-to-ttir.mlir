...
// Test: arith.constant tagged with "rock.sub_byte_shift" is replaced with
// Triton-native ops (tt.make_range + arithmetic + tt.expand_dims + tt.broadcast).
// This prevents Triton GPU passes from hanging on large non-splat constants.

// CHECK-LABEL: @test_sub_byte_shift_constant_lowering
// The tagged constant should be replaced with make_range-based computation.
...
// CHECK: tt.broadcast
func.func @test_sub_byte_shift_constant_lowering(%arg0: tensor<4x4xi8>) -> tensor<4x4xi8> attributes {rock.arch = "##TOKEN_ARCH##", rock.kernel} {
  %shifts = arith.constant {rock.sub_byte_shift = {axis = 1 : i64, halfPeriod = 1 : i64, v0 = 0 : i8, v1 = 4 : i8}} dense<[[0, 4, 0, 4], [0, 4, 0, 4], [0, 4, 0, 4], [0, 4, 0, 4]]> : tensor<4x4xi8>
  %result = arith.shrui %arg0, %shifts : tensor<4x4xi8>
  return %result : tensor<4x4xi8>
}

// -----

// Test: arith.constant tagged with "rock.sub_byte_shift" and halfPeriod > 1.
// Pattern [0, 0, 4, 4, 0, 0, 4, 4] has halfPeriod=2.

// CHECK-LABEL: @test_sub_byte_shift_half_period_2
// CHECK-NOT: rock.sub_byte_shift
// CHECK: tt.make_range
// CHECK: arith.divui
// CHECK: arith.remui
...
// CHECK: tt.expand_dims
// CHECK: tt.broadcast
func.func @test_sub_byte_shift_half_period_2(%arg0: tensor<4x8xi8>) -> tensor<4x8xi8> attributes {rock.arch = "##TOKEN_ARCH##", rock.kernel} {
  %shifts = arith.constant {rock.sub_byte_shift = {axis = 1 : i64, halfPeriod = 2 : i64, v0 = 0 : i8, v1 = 4 : i8}} dense<[[0, 0, 4, 4, 0, 0, 4, 4], [0, 0, 4, 4, 0, 0, 4, 4], [0, 0, 4, 4, 0, 0, 4, 4], [0, 0, 4, 4, 0, 0, 4, 4]]> : tensor<4x8xi8>
  %result = arith.shrui %arg0, %shifts : tensor<4x8xi8>
  return %result : tensor<4x8xi8>
}

// -----

// Test: arith.constant WITHOUT the "rock.sub_byte_shift" attribute is untouched.

// CHECK-LABEL: @test_non_tagged_constant_unchanged
// CHECK: arith.constant dense<
// CHECK-NOT: tt.make_range
func.func @test_non_tagged_constant_unchanged(%arg0: tensor<4x4xi8>) -> tensor<4x4xi8> attributes {rock.arch = "##TOKEN_ARCH##", rock.kernel} {
  %cst = arith.constant dense<[[0, 4, 0, 4], [0, 4, 0, 4], [0, 4, 0, 4], [0, 4, 0, 4]]> : tensor<4x4xi8>
