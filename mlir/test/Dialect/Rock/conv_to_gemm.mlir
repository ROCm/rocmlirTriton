...
func.func @nhwc_1x1_stride_2(%arg0: tensor<16384xf16>, %arg1: tensor<802816xf16>, %arg2: tensor<802816xf16>) -> tensor<64x7x7x1x256xf16> attributes {block_size = 128 : i32, enable_splitk_for_tuning, kernel = 0 : i32, rock.arch = "amdgcn-amd-amdhsa:gfx1100", numCU = 96 : i32} {
  ...
  %result = rock.conv(%0, %1, %2) {derivedBlockSize = 128 : i32, dilations = [1 : index, 1 : index], filter_layout = ["g", "k", "0", "1", "c"], input_layout = ["ni", "0i", "1i", "gi", "ci"], numCU = 96 : i32, output_layout = ["no", "0o", "1o", "go", "ko"], padding = [0 : index, 0 : index, 0 : index, 0 : index], params = #rock.gemm_params<kPerBlock = 4, mPerBlock = 256, nPerBlock = 64, kpack = 1, numWaves = 4, matrixInstrNonkdim = 0, splitKFactor = 1, numStages = 2, wavesPerEU = 0, gridGroupSize = 0, numCTAs = 1>, strides = [2 : index, 2 : index]} : tensor<1x256x1x1x64xf16>, tensor<64x14x14x1x64xf16>, tensor<64x7x7x1x256xf16> -> tensor<64x7x7x1x256xf16>
