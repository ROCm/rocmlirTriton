...
// CHECK-NOT: Embed
// CHECK: rock.gemm
func.func @nhwc_1x1(%arg0: tensor<16384xf16>, %arg1: tensor<802816xf16>, %arg2: tensor<3211264xf16>) -> tensor<64x14x14x1x256xf16> attributes {rock.block_size = 128 : i32, rock.enable_splitk_for_tuning, rock.kernel = 0 : i32, rock.arch = "amdgcn-amd-amdhsa:gfx1100", rock.numCU = 96 : i32} {
...
// CHECK: rock.gemm
func.func @nhwc_1x1_stride_2(%arg0: tensor<16384xf16>, %arg1: tensor<802816xf16>, %arg2: tensor<802816xf16>) -> tensor<64x7x7x1x256xf16> attributes {rock.block_size = 128 : i32, rock.enable_splitk_for_tuning, rock.kernel = 0 : i32, rock.arch = "amdgcn-amd-amdhsa:gfx1100", rock.numCU = 96 : i32} {
  %0 = rock.transform %arg0 by <affine_map<(d0, d1, d2, d3, d4) -> ((d0 * 256 + d1 + d2 + d3) * 64 + d4)> by [<Unmerge{1, 256, 1, 1, 64} ["g", "k", "0", "1", "c"] at [0, 1, 2, 3, 4] -> ["raw"] at [0]>] bounds = [1, 256, 1, 1, 64] -> [16384]> : tensor<16384xf16> to tensor<1x256x1x1x64xf16>
  %1 = rock.transform %arg1 by <affine_map<(d0, d1, d2, d3, d4) -> (((d0 * 14 + d1) * 14 + d2 + d3) * 64 + d4)> by [<Unmerge{64, 14, 14, 1, 64} ["ni", "0i", "1i", "gi", "ci"] at [0, 1, 2, 3, 4] -> ["raw"] at [0]>] bounds = [64, 14, 14, 1, 64] -> [802816]> : tensor<802816xf16> to tensor<64x14x14x1x64xf16>
  ...
  %result = rock.conv(%0, %1) {derivedBlockSize = 128 : i32, dilations = [1 : index, 1 : index], filter_layout = ["g", "k", "0", "1", "c"], input_layout = ["ni", "0i", "1i", "gi", "ci"], rock.numCU = 96 : i32, output_layout = ["no", "0o", "1o", "go", "ko"], padding = [0 : index, 0 : index, 0 : index, 0 : index], params = #rock.gemm_params<kPerBlock = 4, mPerBlock = 256, nPerBlock = 64, kpack = 1, numWaves = 4, matrixInstrNonkdim = 0, splitKFactor = 1, numStages = 2, wavesPerEU = 0, gridGroupSize = 0, numCTAs = 1>, strides = [2 : index, 2 : index]} : tensor<1x256x1x1x64xf16>, tensor<64x14x14x1x64xf16> -> tensor<64x7x7x1x256xf16>
...
// CHECK: rock.gemm
func.func @nhwc_3x3(%arg0: tensor<147456xf16>, %arg1: tensor<802816xf16>, %arg2: tensor<2359296xf16>) -> tensor<64x12x12x1x256xf16> attributes {rock.block_size = 128 : i32, rock.enable_splitk_for_tuning, rock.kernel = 0 : i32, rock.arch = "amdgcn-amd-amdhsa:gfx1100", rock.numCU = 96 : i32} {
...
// CHECK: rock.store
func.func @nhwc_1x1_fptoui_fusion(%arg0: tensor<16384xf32>, %arg1: tensor<802816xf32>, %arg2: tensor<3211264xi32>) -> tensor<64x14x14x1x256xi32> attributes {rock.block_size = 128 : i32, rock.enable_splitk_for_tuning, rock.kernel = 0 : i32, rock.arch = "amdgcn-amd-amdhsa:gfx1100", rock.numCU = 96 : i32} {
