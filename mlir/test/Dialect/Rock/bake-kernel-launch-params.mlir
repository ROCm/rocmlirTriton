// RUN: rocmlir-opt -bake-kernel-launch-params --split-input-file %s | FileCheck %s
...
module attributes {
  ...
  llvm.func @my_kernel(%arg0: !llvm.ptr, %arg1: !llvm.ptr, %arg2: !llvm.ptr) attributes {rock.kernel} {
    llvm.return
  }
}
...
// Verifies both transformations together: static LDS + workspace arg removal.
// CHECK-LABEL: module
// CHECK-NOT: ttg.shared
// CHECK: llvm.mlir.global internal @global_smem(#llvm.undef) {addr_space = 3 : i32, alignment = 16 : i64} : !llvm.array<2048 x i8>
// CHECK: llvm.func @combined(%arg0: !llvm.ptr, %arg1: !llvm.ptr, %arg2: !llvm.ptr)
// CHECK-NOT: ptr<1>
module attributes {
    "ttg.shared" = 2048 : i32,
    "ttg.num-warps" = 4 : i32,
  ...
  llvm.func @combined(%arg0: !llvm.ptr, %arg1: !llvm.ptr, %arg2: !llvm.ptr, %gs: !llvm.ptr<1>, %ps: !llvm.ptr<1>)
      attributes {rock.kernel} {
    llvm.return
  }
}
...
// Verifies that workspace args are NOT removed when the global-scratch arg
// has uses.  The trailing unused profile-scratch arg IS removed.
// CHECK-LABEL: module
// CHECK: llvm.func @used_ws(%arg0: !llvm.ptr, %arg1: !llvm.ptr<1>)
module attributes {
    "ttg.shared" = 0 : i32,
    "ttg.num-warps" = 4 : i32,
    "ttg.threads-per-warp" = 64 : i32
...
// Verifies that at most 2 trailing unused ptr<1> args are removed (the known
// Triton workspace count).  The third ptr<1> from the end is kept even though
// it is unused, because it could be a real data argument.
// CHECK-LABEL: module
...
module attributes {
  ...
  llvm.func @zero_shared(%arg0: !llvm.ptr) attributes {rock.kernel} {
    llvm.return
  }
}
