...
//
//func.func @mlir_dot_add_1(%arg0: tensor<8x32x1xf16>, %arg1: tensor<4x8x16xf16>, %arg2: tensor<16x32xf16>) -> tensor<4x8x32xf16> attributes {rock.arch = "amdgcn-amd-amdhsa:gfx942", kernel} {
...
//  // DISABLED-CHECK: %[[foldA:.*]] = rock.transform %arg1 by {{.*}} : tensor<4x8x16xf16> to tensor<32x16xf16>
//  // DISABLED-CHECK: %[[unbroadcastB:.*]] = rock.transform {{.*}} by {{.*}} : tensor<4x16x32xf16> to tensor<16x32xf16>
//  // DISABLED-CHECK: %[[foldC:.*]] = rock.transform %[[alloc]] by {{.*}} : tensor<4x8x32xf16> to tensor<32x32xf16>
//  // DISABLED-CHECK: %[[gemmOut:.*]] = rock.gemm %[[foldA]] * %[[unbroadcastB]] {{.*}} : tensor<32x16xf16> * tensor<16x32xf16> -> tensor<32x32xf16>
//  // DISABLED-CHECK: %[[untransform:.*]] = rock.tensor_untransform_cast %[[gemmOut]] aka %[[foldC]] : tensor<32x32xf16> to tensor<4x8x32xf16>
//  %5 = rock.gemm %arg1 * %3 features = none : tensor<4x8x16xf16> * tensor<4x16x32xf16> -> tensor<4x8x32xf16>
//  %6 = tensor.empty() : tensor<4x8x32xf16>
//  // DISABLED-CHECK: linalg.generic {{.*}} ins(%[[untransform]], {{.*}}, {{.*}})
...
//func.func @mlir_dot_add_2(%arg0: tensor<8x32x1xf16>, %arg1: tensor<4x8x16xf16>, %arg2: tensor<16x32xf16>) -> tensor<4x8x32xf16> attributes {rock.arch = "amdgcn-amd-amdhsa:gfx942", kernel} {
//  %0 = rock.transform %arg0 by #transform_map : tensor<8x32x1xf16> to tensor<1x8x32xf16>
//  %1 = rock.transform %0 by #transform_map1 : tensor<1x8x32xf16> to tensor<4x8x32xf16>
//
...
//  // DISABLED-CHECK: %[[unbroadcastB:.*]] = rock.transform {{.*}} by {{.*}} : tensor<4x16x32xf16> to tensor<16x32xf16>
//  // DISABLED-CHECK: %[[foldC:.*]] = rock.transform %[[alloc]] by {{.*}} : tensor<4x8x32xf16> to tensor<32x32xf16>
//  // DISABLED-CHECK: %[[gemmOut:.*]] = rock.gemm %[[foldA]] * %[[unbroadcastB]] features = none {perf_config = "v3:16,32,4,16,16,4,4,1,2,1,1"} : tensor<32x16xf16> * tensor<16x32xf16> -> tensor<32x32xf16>
//  // DISABLED-CHECK: %[[untransform:.*]] = rock.tensor_untransform_cast %[[gemmOut]] aka %[[foldC]] : tensor<32x32xf16> to tensor<4x8x32xf16>
//  %5 = rock.gemm %arg1 * %p features = none {perf_config = "v3:16,32,4,16,16,4,4,1,2,1,1"} : tensor<4x8x16xf16> * tensor<4x16x32xf16> -> tensor<4x8x32xf16>
//  %6 = tensor.empty() : tensor<4x8x32xf16>
//  // DISABLED-CHECK: linalg.generic {{.*}} ins(%[[untransform]], {{.*}}, {{.*}})
//  %7 = linalg.generic {indexing_maps = [#map3, #map3, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%5, %1 : tensor<4x8x32xf16>, tensor<4x8x32xf16>) outs(%6 : tensor<4x8x32xf16>) {
//  ^bb0(%in: f16, %in_0: f16, %out: f16):
...
//func.func @mlir_dot_add_3(%arg0: tensor<8x32x1xf16>, %arg1: tensor<4x8x16xf16>, %arg2: tensor<16x32xf16>) -> tensor<4x8x32xf16> attributes {rock.arch = "gfx1100", kernel} {
//  %0 = rock.transform %arg0 by #transform_map : tensor<8x32x1xf16> to tensor<1x8x32xf16>
//  %1 = rock.transform %0 by #transform_map1 : tensor<1x8x32xf16> to tensor<4x8x32xf16>
//
...
//  // DISABLED-CHECK: %[[foldC:.*]] = rock.transform %[[alloc]] by {{.*}} : tensor<3x2x4xf16> to tensor<2x12xf16>
//  // DISABLED-CHECK: %[[gemmOut:.*]] = rock.gemm %[[unbroadcastA]] * %[[foldB]] {{.*}} : tensor<2x3xf16> * tensor<3x12xf16> -> tensor<2x12xf16>
...
//  %2 = rock.gemm %0 * %arg1 features = none : tensor<3x2x3xf16> * tensor<3x3x4xf16> -> tensor<3x2x4xf16>
...
//  // DISABLED-CHECK: %[[gemmOut:.*]] = rock.gemm %[[broadcastA]] * %[[broadcastB]] {{.*}} : tensor<3x2x3xf16> * tensor<3x3x4xf16> -> tensor<3x2x4xf16>
...
//  %3 = rock.gemm %0 * %1 features = none : tensor<3x2x3xf16> * tensor<3x3x4xf16> -> tensor<3x2x4xf16>
...
//  %2 = bufferization.alloc_tensor() : tensor<3x2x4xf16>
//  %3 = rock.gemm %1 * %arg1 features = none : tensor<3x2x3xf16> * tensor<3x3x4xf16> -> tensor<3x2x4xf16>
//  return %3 : tensor<3x2x4xf16>
//}
...
//   // DISABLED-DISABLED-CHECK: %[[foldC:.*]] = rock.transform %[[alloc]] by {{.*}} : tensor<4x8x32xf16> to tensor<32x32xf16>
//   // DISABLED-DISABLED-CHECK: %[[foldScaleA:.*]] = rock.transform %arg2 by {{.*}} : tensor<4x8x16xf8E8M0FNU> to tensor<32x16xf8E8M0FNU>
//   // DISABLED-DISABLED-CHECK: %[[unbroadcastScaleB:.*]] = rock.transform {{.*}} by {{.*}} : tensor<4x16x32xf8E8M0FNU> to tensor<16x32xf8E8M0FNU>
//   // DISABLED-DISABLED-CHECK: %[[gemmOut:.*]] = rock.gemm %[[foldC]] = %[[foldA]] scaled by %[[foldScaleA]] * %[[unbroadcastB]] scaled by %[[unbroadcastScaleB]] {{.*}} : tensor<32x32xf16> = tensor<32x16xf4E2M1FN> scaled by tensor<32x16xf8E8M0FNU> * tensor<16x32xf4E2M1FN> scaled by tensor<16x32xf8E8M0FNU>
...
//   // DISABLED-DISABLED-CHECK: %[[alloc:.*]] = bufferization.alloc_tensor()
//   // DISABLED-DISABLED-CHECK: %[[unbroadcastA:.*]] = rock.transform %[[broadcastA]] by {{.*}} : tensor<3x2x3xf4E2M1FN> to tensor<2x3xf4E2M1FN>
//   // DISABLED-DISABLED-CHECK: %[[foldB:.*]] = rock.transform %arg1 by {{.*}} : tensor<3x3x4xf4E2M1FN> to tensor<3x12xf4E2M1FN>
//   // DISABLED-DISABLED-CHECK: %[[foldC:.*]] = rock.transform %[[alloc]] by {{.*}} : tensor<3x2x4xf16> to tensor<2x12xf16>
//   // DISABLED-DISABLED-CHECK: %[[unbroadcastScaleA:.*]] = rock.transform %[[broadcastScaleA]] by {{.*}} : tensor<3x2x3xf8E8M0FNU> to tensor<2x3xf8E8M0FNU>
//   // DISABLED-DISABLED-CHECK: %[[foldScaleB:.*]] = rock.transform %arg3 by {{.*}} : tensor<3x3x4xf8E8M0FNU> to tensor<3x12xf8E8M0FNU>
//   // DISABLED-DISABLED-CHECK: %[[gemmOut:.*]] = rock.gemm %[[foldC]] = %[[unbroadcastA]] scaled by %[[unbroadcastScaleA]] * %[[foldB]] scaled by %[[foldScaleB]] {{.*}} : tensor<2x12xf16> = tensor<2x3xf4E2M1FN> scaled by tensor<2x3xf8E8M0FNU> * tensor<3x12xf4E2M1FN> scaled by tensor<3x12xf8E8M0FNU>
