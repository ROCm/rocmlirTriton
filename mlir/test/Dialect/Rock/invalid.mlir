...
func.func @transform_unmerge_one_output(%arg0: memref<64xf32>) {
  // expected-error @+2 {{Unmerge can only have one output argument}}
  // expected-error @+1 {{failed to parse Rock_TransformMapAttr parameter 'ops'}}
  %0 = rock.transform %arg0 by <affine_map<(d0, d1) -> (d0, d1)> by [<Unmerge{16, 4} ["a", "b"] at [0, 1] -> ["x", "y"] at [0, 1]>] bounds = [16, 4] -> [16, 4]> : memref<16x4xf32> to memref<16x4xf32>
  return
}
...
func.func @transform_unmerge_lengths(%arg0: memref<64xf32>) {
  // expected-error @+2 {{Unmerge must specify one length per input dimension}}
  // expected-error @+1 {{failed to parse Rock_TransformMapAttr parameter 'ops'}}
  %0 = rock.transform %arg0 by <affine_map<(d0, d1) -> (d0)> by [<Unmerge{16} ["a", "b"] at [0, 1] -> ["x"] at [0]>] bounds = [16, 4] -> [64]> : memref<16x4xf32> to memref<64xf32>
  return
}
