...
// P = streamKMultiple * num_cu workgroups. The partition dimension is chosen
// N -> M -> G: a wave spans the other two dims fully and `span` blocks along
// the partition dim. Output fusion is replicated per cell.
//
// The pass is gated by the `streamKMultiple` tuning parameter (0 = disabled,
// >= 1 = that multiple of num_cu). -canonicalize is used to drop the now-dead
// original gridwise_gemm / fusion / store (the real pipeline DCEs them via
// addWithDCE).
...
func.func @stream_k_skinny_m(%a: tensor<1x128x512xf16>, %b: tensor<1x512x11520xf16>, %c: tensor<1x128x11520xf32>) -> tensor<1x128x11520xf32> attributes {rock.kernel, rock.arch = "amdgcn-amd-amdhsa:gfx942", rock.num_cu = 80 : i32, rock.grid_size = 90 : i32} {
  %r = rock.gridwise_gemm(%a, %b) {params = #params} : tensor<1x128x512xf16>, tensor<1x512x11520xf16> -> tensor<1x128x11520xf32>
...
// Fallback: grid fits within one persistent wave (gridFull = 4 <= P = 80), so
// the pass leaves the gemm unchanged.
...
// splitK = 4 K-slices of 64. The data-parallel wave keeps the real K = 192.
// ============================================================

// CHECK-LABEL: func.func @stream_k_kpad
// CHECK-SAME: rock.grid_size = 80 : i32
// CHECK-DAG: rock.gridwise_gemm{{.*}}tensor<1x1280x192xf16>{{.*}} -> tensor<1x1280x1024xf32>
// CHECK-DAG: Pad{0, 64} ["gemmKPad"]
// CHECK-DAG: rock.gridwise_gemm{{.*}} -> tensor<4x1280x256xf32>
// CHECK-DAG: rock.store{{.*}}by set
// CHECK-DAG: rock.store{{.*}}by atomic_add
func.func @stream_k_kpad(%a: tensor<1x1280x192xf16>, %b: tensor<1x192x1280xf16>, %c: tensor<1x1280x1280xf32>) -> tensor<1x1280x1280xf32> attributes {rock.kernel, rock.arch = "amdgcn-amd-amdhsa:gfx942", rock.num_cu = 80 : i32, rock.grid_size = 100 : i32} {
  %r = rock.gridwise_gemm(%a, %b) {params = #params} : tensor<1x1280x192xf16>, tensor<1x192x1280xf16> -> tensor<1x1280x1280xf32>
  %out = rock.store %r to %c by set : tensor<1x1280x1280xf32> -> tensor<1x1280x1280xf32> to tensor<1x1280x1280xf32>
  return %out : tensor<1x1280x1280xf32>
}
...
// pass searches downward and picks the largest valid P = 156 (N-partition:
// span=26 -> 3328 cols, remBlocks=2, splitK=13). The func grid_size becomes the
// reduced P = 156.
// ============================================================
...
// CHECK-DAG: rock.store{{.*}}by set
// CHECK-DAG: rock.store{{.*}}by atomic_add
func.func @stream_k_smaller_p(%a: tensor<1x768x256xf16>, %b: tensor<1x256x3584xf16>, %c: tensor<1x768x3584xf32>) -> tensor<1x768x3584xf32> attributes {rock.kernel, rock.arch = "amdgcn-amd-amdhsa:gfx942", rock.num_cu = 80 : i32, rock.grid_size = 168 : i32} {
  %r = rock.gridwise_gemm(%a, %b) {params = #params} : tensor<1x768x256xf16>, tensor<1x256x3584xf16> -> tensor<1x768x3584xf32>
