...
func.func @rock_conv(%filter : tensor<?x?x?x?x?xf32>, %input : tensor<?x?x?x?x?xf32>, %output : tensor<?x?x?x?x?xf32>) -> tensor<?x?x?x?x?xf32> attributes {arch = "amdgcn-amd-amdhsa:gfx906"} {
...
func.func @rock_conv_f16(%filter : tensor<?x?x?x?x?xf16>, %input : tensor<?x?x?x?x?xf16>, %output : tensor<?x?x?x?x?xf16>) -> tensor<?x?x?x?x?xf16> attributes {arch = "amdgcn-amd-amdhsa:gfx906"} {
  %result = rock.conv(%filter, %input, %output) features = none {
    filter_layout = ["g" ,"k", "c", "0", "1"],
    input_layout = ["n", "gi", "c", "0i", "1i"],
    output_layout = ["n", "go", "k", "0o", "1o"],
...
func.func @rock_conv_fp8_mixed(%filter : tensor<?x?x?x?x?xf8E4M3FNUZ>, %input : tensor<?x?x?x?x?xf8E5M2FNUZ>, %output : tensor<?x?x?x?x?xf32>) -> tensor<?x?x?x?x?xf32> attributes {arch = "amdgcn-amd-amdhsa:gfx942"} {
  %result = rock.conv(%filter, %input, %output) features = mfma {
    filter_layout = ["g", "k", "c", "0", "1"],
    input_layout = ["n", "gi", "c", "0i", "1i"],
    output_layout = ["n", "go", "k", "0o", "1o"],
    dilations = [1 : index,  1 : index],
    strides = [1 : index,  1 : index],
    padding = [0 : index,  0 : index,  0 : index,  0 : index]
  } : tensor<?x?x?x?x?xf8E4M3FNUZ>, tensor<?x?x?x?x?xf8E5M2FNUZ>, tensor<?x?x?x?x?xf32> -> tensor<?x?x?x?x?xf32>
  return %result : tensor<?x?x?x?x?xf32>
}
// CHECK-LABEL: func.func @rock_conv_fp8_mixed
// CHECK-NEXT: rock.conv

func.func @rock_conv_fp8_mixed_ocp(%filter : tensor<?x?x?x?x?xf8E4M3FN>, %input : tensor<?x?x?x?x?xf8E5M2>, %output : tensor<?x?x?x?x?xf32>) -> tensor<?x?x?x?x?xf32> attributes {arch = "amdgcn-amd-amdhsa:gfx950"} {
  %result = rock.conv(%filter, %input, %output) features = mfma {
    filter_layout = ["g", "k", "c", "0", "1"],
    input_layout = ["n", "gi", "c", "0i", "1i"],
    output_layout = ["n", "go", "k", "0o", "1o"],
    ...
    strides = [1 : index,  1 : index],
    padding = [0 : index,  0 : index,  0 : index,  0 : index]
  } : tensor<?x?x?x?x?xf8E4M3FN>, tensor<?x?x?x?x?xf8E5M2>, tensor<?x?x?x?x?xf32> -> tensor<?x?x?x?x?xf32>
  return %result : tensor<?x?x?x?x?xf32>
}
// CHECK-LABEL: func.func @rock_conv_fp8_mixed
// CHECK-NEXT: rock.conv

func.func @rock_conv_bwd_data(%filter : tensor<?x?x?x?x?xf32>, %input : tensor<?x?x?x?x?xf32>, %output : tensor<?x?x?x?x?xf32>) -> tensor<?x?x?x?x?xf32> attributes {arch = "amdgcn-amd-amdhsa:gfx906"} {
...
func.func @rock_conv_bwd_data_f16(%filter : tensor<?x?x?x?x?xf16>, %input : tensor<?x?x?x?x?xf16>, %output : tensor<?x?x?x?x?xf16>) -> tensor<?x?x?x?x?xf16> attributes {arch = "amdgcn-amd-amdhsa:gfx906"} {
  %result = rock.conv_bwd_data(%filter, %input, %output) features = none {
    filter_layout = ["g", "k", "c", "0", "1"],
    kernelId = 0 : index,
    input_layout = ["n", "gi", "c", "0i", "1i"],
...
func.func @rock_conv_bwd_weight(%filter : tensor<?x?x?x?x?xf32>, %input : tensor<?x?x?x?x?xf32>, %output : tensor<?x?x?x?x?xf32>) -> tensor<?x?x?x?x?xf32> attributes {arch = "amdgcn-amd-amdhsa:gfx906"} {
...
func.func @rock_conv_bwd_weight_f16(%filter : tensor<?x?x?x?x?xf16>, %input : tensor<?x?x?x?x?xf16>, %output : tensor<?x?x?x?x?xf16>) -> tensor<?x?x?x?x?xf16> attributes {arch = "amdgcn-amd-amdhsa:gfx906"} {
  %result = rock.conv_bwd_weight(%filter, %input, %output) features = none {
    filter_layout = ["g", "k", "c", "0", "1"],
    input_layout = ["n", "gi", "c", "0i", "1i"],
    numCU = 64 : i32,
...
func.func @rock_gemm(%a : tensor<32x64xf16>, %b : tensor<1x32x128xf16>) -> tensor<64x128xf32> attributes {arch = "amdgcn-amd-amdhsa:gfx906"} {
...
// func.func @rock_scaled_gemm(%a : tensor<32x64xf4E2M1FN>, %b : tensor<1x32x128xf4E2M1FN>, %scaleA : tensor<32x64xf8E8M0FNU>, %scaleB : tensor<1x32x128xf8E8M0FNU>) -> tensor<64x128xf32> attributes {arch = "amdgcn-amd-amdhsa:gfx950"} {
//   %result = rock.gemm tr %a scaled by tr %scaleA * %b scaled by %scaleB features = mfma storeMethod = set
...
func.func @rock_gridwise_gemm(%A : tensor<2x1024x1024xf32>, %B : tensor<2x1024x2048xf32>) -> tensor<2x1024x2048xf32> attributes {arch = "amdgcn-amd-amdhsa:gfx908", numCU = 64 : i32} {
...
// TODO: Scaled gemm tests need rework
// func.func @rock_gridwise_scaled_gemm(%A : tensor<2x1024x1024xf4E2M1FN>, %B : tensor<2x1024x2048xf4E2M1FN>, %scaleA : tensor<2x1024x1024xf8E8M0FNU>, %scaleB : tensor<2x1024x2048xf8E8M0FNU>) -> tensor<2x1024x2048xf32> attributes {arch = "amdgcn-amd-amdhsa:gfx950", numCU = 256 : i32} {
...
//   %result = rock.blockwise_gemm %matrixC += %bufferA from %matrixA scaled by %bufferScaleA from %matrixScaleA * %bufferB from %matrixB scaled by %bufferScaleB from %matrixScaleB features = mfma {
...
// }
//
// CHECK-LABEL: @rock_blockwise_gemm_scaled
// CHECK-NEXT: rock.blockwise_gemm
...
//   %result = rock.threadwise_gemm_accel %matrixC += %matrixA scaled by %scaleA * %matrixB scaled by %scaleB at [%c0, %c0, %c0] features = mfma{
//     arch = "amdgcn-amd-amdhsa:gfx950",
//     params = #rock.gemm_params<
//       mPerBlock = 256,
//       nPerBlock = 256,
...
// func.func @gridwise_attn_atomic_add(%arg0: tensor<1x384x64xf32>, %arg1: tensor<1x64x384xf32>, %arg2: tensor<1x384x64xf32>, %arg3: tensor<1x384x64xf32>) -> tensor<1x384x64xf32> attributes {block_size = 64 : i32, grid_size = 24 : i32, kernel, mhal.arch = "amdgcn-amd-amdhsa:gfx908:sramecc+:xnack-"} {
...
//     blockSize = 64 : i32,
//     gridSize = 24 : i32,
//     params0 = #rock.gemm_params<kPerBlock = 32, mPerBlock = 32, nPerBlock = 32, numWaves = 1, kpack = 1, matrixInstrNonkdim = 32, splitKFactor = 1, numStages = 2, wavesPerEU = 0, gridGroupSize = 0, numCTAs = 1>,
//     params1 = #rock.gemm_params<kPerBlock = 32, mPerBlock = 32, nPerBlock = 32, numWaves = 1, kpack = 1, matrixInstrNonkdim = 32, splitKFactor = 1, numStages = 2, wavesPerEU = 0, gridGroupSize = 0, numCTAs = 1>,
...
// func.func @attention(%arg0: tensor<1x384x64xf16>, %arg1: tensor<1x384x64xf16>, %arg2: tensor<1x384x64xf16>, %arg3: tensor<1x384x64xf16>) -> tensor<1x384x64xf16> attributes {kernel, mhal.arch = "amdgcn-amd-amdhsa:gfx1100"} {
...
//   } {features = #rock<GemmFeatures dot|atomic_add|atomic_fmax_f32|wmma>, firstGemmIndices = array<i64: 0>, splitKV = 1 : i32, numHeadsKV = 1 : i32, numHeadsQ = 1 : i32, storeMethod = #rock<StoreMethod set>} -> tensor<1x384x64xf16>
