...
module {
  ...
  // DISABLED-CHECK-LABEL: func @mlir_attention
  ...
  // DISABLED-CHECK: arith.muli %{{.*}}, %c32{{.*}} : index
  // DISABLED-CHECK: %[[MAX_ROW:.*]] = arith.subi %{{.*}}, %c1{{.*}} : index
  // The prefixOffset is loaded from tensor and converted to index
  // DISABLED-CHECK: %[[OFFSET:.*]] = arith.index_cast %{{.*}} : i32 to index
  // effectiveSeqLen = maxRowOfBlock + prefixOffset
  // DISABLED-CHECK: %[[EFFECTIVE_SEQ_UNBOUND:.*]] = arith.addi %[[MAX_ROW]], %[[OFFSET]] : index
  // Bound by gemm0M - 1 (key sequence length - 1) to prevent out-of-bounds access
  // DISABLED-CHECK: %[[EFFECTIVE_SEQ:.*]] = arith.minui %[[EFFECTIVE_SEQ_UNBOUND]], %c{{.*}} : index
  // Ceiling division: (effectiveSeqLen + blockSize) / blockSize
  // DISABLED-CHECK: arith.addi %[[EFFECTIVE_SEQ]], %c32{{.*}} : index
  // DISABLED-CHECK: %[[END:.*]] = arith.divui %{{.*}}, %c32{{.*}} : index

  // Verify the main loop uses the computed end bound
  // DISABLED-CHECK: scf.for %{{.*}} = %c0{{.*}} to %[[END]] step %c1{{.*}}

  // Verify prefix causal masking: key_pos > query_pos + prefixOffset
  // DISABLED-CHECK: %[[COL_PLUS_OFFSET:.*]] = arith.addi %{{.*}}, %[[OFFSET]] : index
  // DISABLED-CHECK: %[[MASK_COND:.*]] = arith.cmpi ugt, %{{.*}}, %[[COL_PLUS_OFFSET]] : index
  // DISABLED-CHECK: scf.if %[[MASK_COND]]
  // DISABLED-CHECK:   rock.in_bounds_store %{{.*}} -> %{{.*}}
  ...
  //     rock.yield
  //   } {causal, operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 1, 1, 0>, params0 = #accel_gemm_params, params1 = #accel_gemm_params, prePadG0M = 16 : index, prePadG0N = 4 : index, softmaxType = f32, splitKV = 1 : i32, storeMethod = #rock<StoreMethod set>} : memref<14x64x32xf16>, memref<14x64x32xf16>, memref<14x32x64xf16>, memref<14xi32>, memref<14x32x64xf16>
