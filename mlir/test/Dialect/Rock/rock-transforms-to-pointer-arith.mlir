...
func.func @test_embed_conv_style(%arg0: tensor<1048576xf32>) -> tensor<8x64xf32> attributes {rock.arch = "##TOKEN_ARCH##"} {
  ...
  %3 = rock.transform %2 by <affine_map<(d0, d1, d2) -> (d2 floordiv 1024, d0, d1 floordiv 9, (d1 floordiv 3) mod 3, (d2 floordiv 32) mod 32, d1 mod 3, d2 mod 32)> by [<PassThrough ["gemmG"] at [0] -> ["gi"] at [1]>, <Merge{8, 3, 3} ["gemmK"] at [1] -> ["ci", "0", "1"] at [2, 3, 5]>, <Merge{128, 32, 32} ["gemmN"] at [2] -> ["ni", "0o", "1o"] at [0, 4, 6]>] bounds = [1, 72, 131072] -> [128, 1, 8, 3, 32, 3, 32]> : tensor<128x1x8x3x32x3x32xf32> to tensor<1x72x131072xf32>
