...
// CHECK-LABEL: @test_coords_to_ptr_load
// CHECK-SAME: (%[[ARG0:.*]]: tensor<8xi8>
//      CHECK:   %[[BASE_PTR:.*]] = rock.extract_ptr %[[ARG0]] : tensor<8xi8> -> i32
//  CHECK-NOT:   arith.divui
//  CHECK-NOT:   arith.remui
//      CHECK:   arith.cmpi ult
//      CHECK:   tt.splat %[[BASE_PTR]] : i32 -> tensor<2x4xi32>
//      CHECK:   arith.addi {{.*}} : tensor<2x4xi32>
//      CHECK:   rock.blockwise_load_ptr {{.*}} : tensor<2x4xi32>, tensor<2x4xi1> -> tensor<2x4xi8>
//  CHECK-NOT:   rock.coords_to_ptr
func.func @test_coords_to_ptr_load(%arg0: tensor<8xi8>, %ci: tensor<2x1xi32>, %tap: tensor<2x1xi32>, %oo: tensor<1x4xi32>) -> tensor<2x4xi8> attributes {rock.arch = "##TOKEN_ARCH##"} {
  %c0_i32 = arith.constant 0 : i32
  ...
  %pointers, %mask = rock.coords_to_ptr %2[%c0_i32, %c0_i32, %ci, %tap, %oo] : tensor<1x1x2x3x4xi8>, i32, i32, tensor<2x1xi32>, tensor<2x1xi32>, tensor<1x4xi32> -> tensor<2x4xi32>, tensor<2x4xi1>
  %3 = rock.blockwise_load_ptr %pointers[%mask] {cacheModifier = #rock<CacheModifier none>} : tensor<2x4xi32>, tensor<2x4xi1> -> tensor<2x4xi8>
...
// coords_to_ptr whose transform contains a non IV Merge: the
// coordinate operand for the merged dim must be re-decomposed by the lowering,
// so div/mod DO appear here (unlike @test_coords_to_ptr_load). This confirms
// coords_to_ptr does not assume the below-chain is merge-free.

// CHECK-LABEL: @test_coords_to_ptr_merge_below
// CHECK-SAME: (%[[ARG0:.*]]: tensor<16xi8>
//      CHECK:   rock.extract_ptr %[[ARG0]]
//      CHECK:   arith.divui
//      CHECK:   arith.remui
//      CHECK:   arith.addi {{.*}} : tensor<2x8xi32>
//      CHECK:   rock.blockwise_load_ptr {{.*}} : tensor<2x8xi32>, tensor<2x8xi1> -> tensor<2x8xi8>
//  CHECK-NOT:   rock.coords_to_ptr
func.func @test_coords_to_ptr_merge_below(%arg0: tensor<16xi8>, %n: tensor<2x1xi32>, %k: tensor<1x8xi32>) -> tensor<2x8xi8> attributes {rock.arch = "##TOKEN_ARCH##"} {
  // Buffer chain: raw(16) -> unmerge (n=2, a=2, b=4) -> Merge (a, b) into k=8.
  %0 = rock.transform %arg0 by <affine_map<(d0, d1, d2) -> ((d0 * 2 + d1) * 4 + d2)> by [<Unmerge{2, 2, 4} ["n", "a", "b"] at [0, 1, 2] -> ["raw"] at [0]>] bounds = [2, 2, 4] -> [16]> : tensor<16xi8> to tensor<2x2x4xi8>
  %1 = rock.transform %0 by <affine_map<(d0, d1) -> (d0, d1 floordiv 4, d1 mod 4)> by [<PassThrough ["n"] at [0] -> ["n"] at [0]>, <Merge{2, 4} ["k"] at [1] -> ["a", "b"] at [1, 2]>] bounds = [2, 8] -> [2, 2, 4]> : tensor<2x2x4xi8> to tensor<2x8xi8>

  // Seed at the merged space; the lowering re-decomposes k through the Merge.
  %pointers, %mask = rock.coords_to_ptr %1[%n, %k] : tensor<2x8xi8>, tensor<2x1xi32>, tensor<1x8xi32> -> tensor<2x8xi32>, tensor<2x8xi1>
  %2 = rock.blockwise_load_ptr %pointers[%mask] {cacheModifier = #rock<CacheModifier none>} : tensor<2x8xi32>, tensor<2x8xi1> -> tensor<2x8xi8>

  return %2 : tensor<2x8xi8>
}
