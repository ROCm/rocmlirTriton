...
// binary and the launch, so a gemm with a dynamic G, M or N dimension has to be
// rejected with a diagnostic instead of computing a nonsensical grid.
...
func.func @gemm_dynamic_m(%a: tensor<1x?x72xf32>, %b: tensor<1x72x64xf32>, %c: tensor<1x?x64xf32>) -> tensor<1x?x64xf32> attributes {rock.arch = "amdgcn-amd-amdhsa:gfx942"} {
  // expected-error @+2 {{cannot compute a static grid size for a gemm with dynamic G, M or N dimensions}}
  // expected-error @+1 {{failed to legalize operation 'rock.gemm'}}
  %result = rock.gemm %a * %b {
    params = #gemm_params
  } : tensor<1x?x72xf32> * tensor<1x72x64xf32> -> tensor<1x?x64xf32>
