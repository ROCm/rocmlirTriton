...
// finite row-max sentinel and one row-vector denominator clamp, without
// per-element compare/select guards on the score tile.
// NO-CMP-LABEL: func @gridwise_attn_causal_sliding
// NO-CMP-NOT: arith.cmpf
// NO-CMP: return
// CHECK-LABEL: func @gridwise_attn_causal_sliding
// CHECK-DAG: %[[LOWEST:.+]] = arith.constant dense<-3.40282347E+38> : tensor<16xf32>
// CHECK-DAG: %[[ZERO_ROW:.+]] = arith.constant dense<0.000000e+00> : tensor<16xf32>
// CHECK-DAG: %[[ONE:.+]] = arith.constant dense<1.000000e+00> : tensor<16xf32>
// CHECK: %[[OUT:.+]]:3 = scf.for
// CHECK-SAME: %[[RMAX:.+]] = %[[LOWEST]]
// CHECK-SAME: %[[RSUM:.+]] = %[[ZERO_ROW]]
// CHECK: %[[SUB:.+]] = arith.subf
// CHECK-NEXT: %[[P:.+]] = math.exp2 %[[SUB]]
// CHECK: %[[SAFE_SUM:.+]] = arith.maxnumf %[[OUT]]#2, %[[ONE]]
// CHECK: %[[SUM_EXP:.+]] = tt.expand_dims %[[SAFE_SUM]]
// CHECK: arith.divf
...
// CHECK-LABEL: func @gridwise_attn_causal_sliding_f16
// CHECK: arith.constant dense<{{(-6.550400e\+04|0xFBFF)}}> : tensor<16xf16>
// CHECK: arith.maxnumf
...
// Identity plus |scale| <= 1 compile-time scaling cannot overflow finite QK
// scores: retain the original -inf running maximum and add no denominator
// clamp.
// CHECK-LABEL: func @gridwise_attn_finite_scale
// CHECK: arith.constant dense<0xFF800000> : tensor<16xf32>
// CHECK-NOT: arith.maxnumf
// CHECK: return
...
// CHECK-LABEL: func @gridwise_attn_causal_only
// CHECK: arith.constant dense<0xFF800000> : tensor<16xf32>
// CHECK-NOT: arith.maxnumf
// CHECK: return
...
// CHECK-LABEL: func @gridwise_attn_split_kv
// CHECK: arith.constant dense<-3.40282347E+38> : tensor<64xf32>
// CHECK: arith.maxnumf
// CHECK: return
