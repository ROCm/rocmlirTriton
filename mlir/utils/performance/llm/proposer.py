...
if __package__:
    ...
    from . import feedback, prompting, transport
else:
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    from llm import configs as config_utils
    from llm import feedback, prompting, transport
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--session",
        default="",
        help="where the conversation is kept between rounds; a debug artifact "
        "as much as a mechanism",
    )
    parser.add_argument(
        "--print-prompt",
        action="store_true",
    ...
    session.setdefault("rounds", []).append({"round": request.get("round"), "prompt": prompt})
