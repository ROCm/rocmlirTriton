...
_KERNEL_NOTES = {
    "Gemm":
    "A batched matrix multiply: C[G,M,N] = A[G,M,K] x B[G,K,N]. The "
    "dimensions are the user's own, so no dimension is systematically larger "
    "than another.",
    "Conv":
    "A forward convolution, lowered to an implicit GEMM, so the dimensions "
    ...
    "ConvBwdData":
    "The data gradient of a convolution, lowered to an implicit GEMM: M is "
    ...
    "Attention":
    "Fused attention: gemm0 is Q x K^T and gemm1 is softmax(gemm0) x V, with "
...
_FUSED_TILING_NOTE = (
    "One perf config tiles both GEMMs: mPerBlockG0 and nPerBlockG0 tile "
...
def describe_problem(problem: Problem) -> str:
    ...
    if problem.get("quantBlockSize") is not None:
        lines.append(
            f"  Block-scaled GEMM: {problem['quantBlockSize']} elements per scale, "
...
def _describe_layout(problem: Problem) -> List[str]:
    ...
    lines = [
        "  Operand layout: "
        + (", ".join(f"{name} transposed" for name in transposed)
           if transposed else "none transposed")
        + f" (of {', '.join(name for name, _ in stated)})."
    ]
    if transposed:
        lines.append(_note(
            "A transposed operand has its reduction (K) dimension contiguous "
...
def _describe_convolution(problem: Problem) -> List[str]:
    ...
    window = [
        f"{label}={problem[key]}"
        for label, key in (("strides", "strides"), ("dilations", "dilations"),
                           ("padding", "padding"))
        if problem.get(key)
    ]
    if window:
    ...
    if lines:
        lines.append(_note(
            "The GEMM dimensions above are this convolution's implicit-GEMM "
    ...
    if alignment > 1:
        lines.append(_note(
            f"K here is a merge of the input channels and the filter's "
...
def _describe_fusion(problem: Problem) -> List[str]:
    ...
    if problem.get("hasPreSecondGemmFusion"):
        ...
        lines.append(
            f"  An elementwise operation is fused between the two GEMMs, "
            f"reading {inputs} extra tensor(s) -- for attention this is the "
            "usual scale and/or bias applied before the softmax."
        )
        lines.append(_note(
            "Its operands are live across the first GEMM's accumulator, so "
    ...
    if problem.get("hasFusedReduction"):
        lines.append(_note(
            "The kernel also reduces this GEMM's output. That reduction "
...
def _describe_masking(problem: Problem) -> List[str]:
    ...
    if heads_kv and heads_q > heads_kv:
        lines.append(_note(
            f"Grouped-query attention: {heads_q // heads_kv} query heads share "
    ...
    if problem.get("causal"):
        lines.append(_note(
            "Causal masking: a query only attends to keys at or before its own "
    ...
    if problem.get("slidingWindowLookBack") is not None:
        lines.append(
            f"  Sliding-window attention with a look-back of "
    ...
    if problem.get("hasLastValidKVIndex"):
        lines.append(
            "  The valid K/V extent is a run-time value, so the loop over N is "
    ...
    if split_kv > 1:
        lines.append(_note(
            f"Flash decoding with splitKV={split_kv}: the grid is multiplied by "
    ...
    if problem.get("hasLse"):
        lines.append(
            "  The log-sum-exp output is written as well, which is another "
...
def describe_hardware(hardware: Hardware) -> str:
    ...
    capability.append(
        "asynchronous (direct-to-LDS) global loads available"
        if hardware.get("supportsAsyncCopy")
    ...
    capability.append(
        "kPerBlock need not be a power of two"
        if hardware.get("supportsNonPow2KPerBlock")
        else "kPerBlock must be a power of two"
    )
    ...
    if hardware.get("preferBf16x3ForF32Dot"):
        lines.append(_note(
            "One heuristic, rather than a property of the chip: this family "
...
def compute_workload_hints(problem: Problem, hardware: Hardware) -> List[str]:
    ...
    return _gemm_hints(m, n, k, num_cus, hardware,
                       problem.get("kPerBlockAlignment") or 1)
...
def _gemm_hints(m: int, n: int, k: int, num_cus: int, hardware: Hardware,
    ...
    for tile in (64, 128, 256):
        ...
        if tiles >= num_cus:
            hints.append(
                f"A {tile}x{tile} output tile gives {tiles} workgroups, which fills "
        ...
        hints.append(
            f"Even a 64x64 output tile gives only {tiles} workgroups against "
    ...
    if k >= 8 * max(m, n):
        hints.append(
            f"K ({k}) dwarfs both M ({m}) and N ({n}): this is a skinny GEMM, and "
            "the useful families here are a deep kPerBlock and splitKFactor "
            "above 1."
        )
    elif m >= 8 * n or n >= 8 * m:
        hints.append(
            f"M ({m}) and N ({n}) are very different sizes, so a square tile "
    ...
    if hardware.get("numChiplets", 1) > 1:
        hints.append(
            f"This chip has {hardware['numChiplets']} chiplets, each with its own "
    ...
    if k <= 64:
        hints.append(
            f"K is only {k}, so kPerBlock cannot usefully exceed it and deep "
    ...
    if k_alignment > 1:
        divides = [
            tile for tile in range(k_alignment, min(k, 512) + 1, k_alignment)
            if k % tile == 0
        ]
        hints.append(
            f"This convolution's K is its input channels times a filter "
            ...
            + (f", the best of them also dividing K ({k}) exactly: "
               f"{', '.join(str(tile) for tile in divides[-4:])}."
               if divides else ".")
        )
    return hints
...
def _fused_hints(g: int, m: int, n: int, k: int, o: int, num_cus: int) -> List[str]:
    ...
    for tile in (32, 64, 128, 256):
        ...
        if groups >= num_cus:
            hints.append(
                f"The grid is G x (M / mPerBlockG0), so mPerBlockG0={tile} gives "
        ...
        groups = g * ((m + 31) // 32)
        hints.append(
            f"Even mPerBlockG0=32 gives only {groups} workgroups against "
    ...
    hints.append(
        f"O is {o}, so nPerBlockG1=0 (untiled, the whole of O in one workgroup) "
        "is the usual choice; tiling O only pays once O is large enough that a "
        "whole row of it crowds out LDS or registers."
        if o <= 128 else
        f"O is {o}, which is large enough that nPerBlockG1=0 (the whole of O at "
        "once) may not fit; tiling O is worth trying here."
    )

    if n >= 8 * m:
        hints.append(
            f"N ({n}) dwarfs M ({m}): most of the time goes into the loop over "
    ...
    if k <= 64:
        hints.append(
            f"K is only {k}, which is normal for this kernel shape: it is a head "
