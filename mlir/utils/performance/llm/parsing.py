...
Ported unchanged from Helion's `helion/autotuner/llm/parsing.py`. Every
function here exists to handle a specific way a model gets JSON wrong -- Python
literals instead of JSON ones, a fenced code block, a sentence of prose either
side of the object -- and none of it is specific to what is being tuned, so
there is nothing to adapt and every reason to keep it recognizable against
upstream.
"""
