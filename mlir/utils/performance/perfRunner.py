...
DEBUG_HEADER_TO_FLAG = {
    ...
    'ScaledGemm': '-scaledGemm',
    'ScaleADtype': '-scale_a_dtype',
    'ScaleBDtype': '-scale_b_dtype',
    'SplitKV': '-split_kv',
    'SeqLenK': '-seq_len_k',
    'SeqLenQ': '-seq_len_q',
    ...
    'TransO': '-transO',
    'TransQ': '-transQ',
    'TransScaleA': '-transScaleA',
    'TransScaleB': '-transScaleB',
    'TransV': '-transV',
    'W': '-w',
...
def parse_debug_db_row(row) -> str:
    ...
    # Filter out Chip, numCU, and numChiplets values as they are already
    # accounted for and we do not want to double count them
    args = []
    for key, value in row.items():
        if key in DEBUG_HEADER_TO_FLAG and key not in ("Chip", "numCU", "numChiplets"):
...
class AttentionConfiguration(PerfConfiguration):
    ...
    def get_total_flops(self, only_matmul_flops=True):
