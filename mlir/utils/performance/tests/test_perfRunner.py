...
class TestParseTuningDbLine:
    ...
    All three on-disk formats collapse to the same (arch, config, perfconfig) triple: the
    num_cu/num_chiplets columns are accepted for forward compatibility but dropped, because
    lookups here are keyed on (arch, config) alone.
    """

    def test_legacy_three_entries(self):
        out = perfRunner.parse_tuning_db_line(["gfx900", "config1", "perf1"])
        assert out == ("gfx900", "config1", "perf1")
    ...
    def test_v3_extra_columns_are_ignored(self):
        ...
        assert out == ("gfx90x", "gemm -m 1024", "perf_x")

    def test_non_numeric_third_column_is_read_as_v2(self):
        """A 5-column row whose third field is not a number is v2 plus a trailing metric,
        not v3; the config must come from column 3 rather than column 4."""
...
class TestReadTuningDb:
    ...
    def test_read_with_header_and_comments(self):
        # Written without -transO; canonicalization adds it, and the key is the canonical form.
        ...
        try:
            ...
            assert len(db) == 2
            assert db[("gfx900", gemm_a_key)] == "perf_1"
            assert db[("gfx900", gemm_b_key)] == "perf_2"
        finally:
            os.unlink(path)
    ...
    def test_read_skips_unparseable_entries(self):
        ...
        try:
            db = perfRunner.read_tuning_db(path,
                                           ...
                                           fallback_num_chiplets=1)
            assert len(db) == 1
            assert db[("gfx900", valid_gemm_key)] == "perf_ok"
        finally:
            os.unlink(path)
