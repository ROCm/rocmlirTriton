...
def find_rocmlir_build(script_dir: Path) -> Path:
    ...
    candidates = [repo_root / 'build', Path.cwd().parent, Path.cwd(),
                  Path.cwd() / 'build', Path.home() / 'rocmlir' / 'build']
    for d in candidates:
...
def main() -> int:
    ...
    cmd = [str(runner), '-O2', f'--shared-libs={shared_libs}',
           '--entry-point-result=void', *sys.argv[1:]]
