...
struct BenchmarkResult {
  ...
  bool measured() const {
    return status == Status::Success && std::isfinite(timeNs);
  }
};

/// The available search strategies. The first three enumerate a fixed space
/// (see `TuningParamSetKind`); `LFBO` searches adaptively.
...
enum class LFBOEffort : uint32_t {
  Quick = 0,
  Full = 1,
};

/// Maps the names used on command lines ("quick", "full", "exhaustive",
/// "lfbo") onto their enumerator. Returns std::nullopt for anything else.
...
///
/// `effort` sizes the budget of an adaptive search (`LFBO`); a fixed space is
/// as large as it is and ignores it.
///
/// `tracePath` is a debugging aid and nothing a search does depends on it:
/// when it is not empty, a strategy that searches adaptively (`LFBO`) writes
/// a record of each of its iterations there so that a run can be examined
/// afterwards. The strategies that enumerate a fixed space have no iterations
/// to report and ignore it.
std::unique_ptr<TuningSearchStrategy>
createTuningSearchStrategy(ModuleOp mod, SearchStrategyKind kind,
                           LFBOEffort effort = LFBOEffort::Full,
                           StringRef tracePath = "");
