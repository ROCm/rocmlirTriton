...
/// Returns true if the final lower coordinates produced by `transforms` depend
/// on any of `dims` in the upper coordinate space. An empty dimension set
/// returns false, and an empty transform chain returns true conservatively,
/// since it passes its coordinates through unchanged. Every dim in `dims` must
/// be an upper coordinate of a non-empty chain, which is asserted.
bool transformChainDependsOnAnyDim(ArrayRef<TransformMapAttr> transforms,
                                   ArrayRef<unsigned> dims);

/// Returns true if a validity check generated by `transforms` at or below
...
/// to express the checked coordinates in that upper space. An empty dimension
/// set or an empty transform chain returns false, the latter because such a
/// chain generates no validity checks. Every dim in `dims` must be an upper
/// coordinate of a non-empty chain, and `firstTransform` must be within it;
/// both are asserted.
