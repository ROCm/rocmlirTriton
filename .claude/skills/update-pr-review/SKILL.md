...
(`review-rocmlir-triton-pr`) already produced a fresh list of findings from the diff. Your job
...
This skill runs in the same two modes as `review-rocmlir-triton-pr`; the available
  ...
  "inline_comments": [
    {
      ...
      "severity": "Major",
      "body": "...",
      "suggestion": "...optional verbatim single-line replacement; see review-rocmlir-triton-pr SKILL.md for the contract..."
    }
  ],
  "thread_updates": [
    {
...
- `verdict` MUST be one of `APPROVE`, `REQUEST_CHANGES`, `COMMENT`, reflecting
  ...
  `/review-rocmlir-triton-pr`). The post job submits every verdict as `--comment`;
  emit the honest verdict.
- `summary` MUST be Markdown formatted with `##` sections per the
  `review-rocmlir-triton-pr` contract -- pass through the upstream `summary`
...
- Pass through the optional `suggestion` field unchanged from the fresh findings;
  ...
  defined by `review-rocmlir-triton-pr` (single-line, verbatim, self-contained, high
