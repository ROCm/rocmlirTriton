...
static bool swizzlesInsideWarp(RankedTensorType srcTy,
  ...
  // offset delta by the vector size to turn it into a lane offset. That vector
  // is the load's, so it is capped by the source's per-thread contiguity and by
  // the widths the target can write to LDS, not by enc.getVec().
  unsigned loadContig = std::min<unsigned>(
      enc.getVec(), triton::gpu::getContigPerThread(srcTy)[enc.getOrder()[0]]);
  int64_t vec = triton::AMD::fitToValidDirectToLdsVecSize(
      loadContig, srcTy.getElementTypeBitWidth(), targetInfo);
...
clampSwizzleForDirectToLds(RankedTensorType srcTy, ttg::SharedEncodingTrait enc,
  ...
  for (unsigned maxPhase = swizz.getMaxPhase(); maxPhase > 1; maxPhase /= 2) {
    ...
    if (swizzlesInsideWarp(srcTy, candidate, targetInfo)) {
      if (candidate != swizz)
