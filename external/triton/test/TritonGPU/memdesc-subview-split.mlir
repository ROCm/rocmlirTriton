...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 8 : i32, ttg.target = "hip:gfx942", "ttg.threads-per-warp" = 64 : i32} {
  ...
  // these 8-row tiles each hold a different phase: an 8-row tile lands at
  // 1024 ^ 32, two bits rather than one, which is a split the static form
  // cannot describe and the index can. Both bits clear the 8-element f16
  // vector the lowering may use, which is what makes it addressable.
  ...
  // so a piece lands at 32 ^ 4, two bits in the physical offset rather than
  // one. The static form cannot say that, but an index can, and 36 clears the
  // 4-element f32 vector the lowering may use.
  // CHECK-LABEL: memdesc_subslice_across_swizzling_pattern
  tt.func public @memdesc_subslice_across_swizzling_pattern(%arg0: !ttg.memdesc<8x16xf32, #crossing, #smem>, %i: i32) {
    // CHECK: ttg.memdesc_subslice %{{.*}}[0, 0] index %{{.*}} : !ttg.memdesc<8x16xf32, {{.*}}> -> !ttg.memdesc<8x4xf32, {{.*}}>
    %a = ttg.memdesc_subslice %arg0 [0, 0] index %i : !ttg.memdesc<8x16xf32, #crossing, #smem> -> !ttg.memdesc<8x4xf32, #crossing, #smem, 8x16>
    tt.return
  }
}
