...
// CHECK-LABEL: gelu_32_accumulators
// CHECK:       %[[X:.+]] = tt.dot
// CHECK-NOT:   math.erf
// CHECK:       tt.store %{{.*}}, %[[X]] {amdg.looped_gelu = {
// CHECK-SAME:  bias = 1.000000e+00 : f32
// CHECK-SAME:  scale = 0.707106769 : f32
#blocked = #ttg.blocked<{sizePerThread = [1, 1], threadsPerWarp = [8, 4], warpsPerCTA = [1, 1], order = [1, 0]}>
#dot_blocked = #ttg.blocked<{sizePerThread = [1, 4], threadsPerWarp = [8, 4], warpsPerCTA = [1, 1], order = [1, 0]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.target = "hip:gfx1201", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @gelu_32_accumulators(%arg0: !tt.ptr<f32>) {
    %zero = arith.constant dense<0.000000e+00> : tensor<32x32xf32, #dot_blocked>
    %a = arith.constant dense<1.000000e+00> : tensor<32x16xf32, #ttg.dot_op<{opIdx = 0, parent = #dot_blocked}>>
    %b = arith.constant dense<1.000000e+00> : tensor<16x32xf32, #ttg.dot_op<{opIdx = 1, parent = #dot_blocked}>>
    %scale = arith.constant dense<7.07106769E-1> : tensor<32x32xf32, #blocked>
    %bias = arith.constant dense<1.000000e+00> : tensor<32x32xf32, #blocked>
    %dot = tt.dot %a, %b, %zero : tensor<32x16xf32, #ttg.dot_op<{opIdx = 0, parent = #dot_blocked}>> * tensor<16x32xf32, #ttg.dot_op<{opIdx = 1, parent = #dot_blocked}>> -> tensor<32x32xf32, #dot_blocked>
    %x = ttg.convert_layout %dot : tensor<32x32xf32, #dot_blocked> -> tensor<32x32xf32, #blocked>
    %scaled = arith.mulf %x, %scale : tensor<32x32xf32, #blocked>
    %erf = math.erf %scaled : tensor<32x32xf32, #blocked>
    %sum = arith.addf %erf, %bias : tensor<32x32xf32, #blocked>
    %gelu = arith.mulf %x, %sum : tensor<32x32xf32, #blocked>
    %ptr = tt.splat %arg0 : !tt.ptr<f32> -> tensor<32x32x!tt.ptr<f32>, #blocked>
    tt.store %ptr, %gelu : tensor<32x32x!tt.ptr<f32>, #blocked>
    tt.return
  }
}
...
// CHECK-LABEL: gelu_64_accumulators_scaled
// CHECK:       %[[X:.+]] = tt.dot
// CHECK-NOT:   math.erf
// CHECK:       tt.store %{{.*}}, %[[X]] {amdg.looped_gelu = {
// CHECK-SAME:  core_fastmath = #arith.fastmath<reassoc>
// CHECK-SAME:  core_is_lhs = false
// CHECK-SAME:  erf_fastmath = #arith.fastmath<nnan>
// CHECK-SAME:  erf_is_lhs = false
// CHECK-SAME:  output_fastmath = #arith.fastmath<afn>
// CHECK-SAME:  output_scale = 5.000000e-01 : f32
// CHECK-SAME:  scale_fastmath = #arith.fastmath<contract>
// CHECK-SAME:  scaled_x_is_lhs = false
// CHECK-SAME:  x_is_lhs = false
#blocked = #ttg.blocked<{sizePerThread = [1, 1], threadsPerWarp = [8, 4], warpsPerCTA = [1, 1], order = [1, 0]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.target = "hip:gfx1201", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @gelu_64_accumulators_scaled(%arg0: !tt.ptr<f32>) {
    %zero = arith.constant dense<0.000000e+00> : tensor<64x32xf32, #blocked>
    %a = arith.constant dense<1.000000e+00> : tensor<64x16xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked}>>
    %b = arith.constant dense<1.000000e+00> : tensor<16x32xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked}>>
    %scale = arith.constant dense<7.07106769E-1> : tensor<64x32xf32, #blocked>
    %bias = arith.constant dense<1.000000e+00> : tensor<64x32xf32, #blocked>
    %half = arith.constant dense<5.000000e-01> : tensor<64x32xf32, #blocked>
    %x = tt.dot %a, %b, %zero : tensor<64x16xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked}>> * tensor<16x32xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked}>> -> tensor<64x32xf32, #blocked>
    %scaled = arith.mulf %scale, %x fastmath<contract> : tensor<64x32xf32, #blocked>
    %erf = math.erf %scaled fastmath<nnan> : tensor<64x32xf32, #blocked>
    %sum = arith.addf %bias, %erf fastmath<nnan> : tensor<64x32xf32, #blocked>
    %core = arith.mulf %sum, %x fastmath<reassoc> : tensor<64x32xf32, #blocked>
    %gelu = arith.mulf %half, %core fastmath<afn> : tensor<64x32xf32, #blocked>
    %ptr = tt.splat %arg0 : !tt.ptr<f32> -> tensor<64x32x!tt.ptr<f32>, #blocked>
    tt.store %ptr, %gelu : tensor<64x32x!tt.ptr<f32>, #blocked>
    tt.return
  }
}
...
// CHECK-LABEL: gelu_128_accumulators_masked
// CHECK:       %[[X:.+]] = scf.for
// CHECK:       tt.dot
// CHECK-NOT:   math.erf
...
#blocked = #ttg.blocked<{sizePerThread = [1, 1], threadsPerWarp = [8, 4], warpsPerCTA = [1, 1], order = [1, 0]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.target = "hip:gfx1201", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @gelu_128_accumulators_masked(%arg0: !tt.ptr<f32>) {
    %zero = arith.constant dense<0.000000e+00> : tensor<64x64xf32, #blocked>
    %a = arith.constant dense<1.000000e+00> : tensor<64x16xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked}>>
    %b = arith.constant dense<1.000000e+00> : tensor<16x64xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked}>>
    %scale = arith.constant dense<7.07106769E-1> : tensor<64x64xf32, #blocked>
    %bias = arith.constant dense<1.000000e+00> : tensor<64x64xf32, #blocked>
    %mask = arith.constant dense<true> : tensor<64x64xi1, #blocked>
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %x = scf.for %iv = %c0 to %c1 step %c1 iter_args(%acc = %zero) -> tensor<64x64xf32, #blocked> {
      %dot = tt.dot %a, %b, %acc : tensor<64x16xf32, #ttg.dot_op<{opIdx = 0, parent = #blocked}>> * tensor<16x64xf32, #ttg.dot_op<{opIdx = 1, parent = #blocked}>> -> tensor<64x64xf32, #blocked>
      scf.yield %dot : tensor<64x64xf32, #blocked>
    }
    %scaled = arith.mulf %x, %scale : tensor<64x64xf32, #blocked>
    %erf = math.erf %scaled : tensor<64x64xf32, #blocked>
    %sum = arith.addf %erf, %bias : tensor<64x64xf32, #blocked>
    %gelu = arith.mulf %x, %sum : tensor<64x64xf32, #blocked>
    %selected = arith.select %mask, %gelu, %zero : tensor<64x64xi1, #blocked>, tensor<64x64xf32, #blocked>
    %ptr = tt.splat %arg0 : !tt.ptr<f32> -> tensor<64x64x!tt.ptr<f32>, #blocked>
    tt.store %ptr, %selected, %mask : tensor<64x64x!tt.ptr<f32>, #blocked>
    tt.return
  }
}
