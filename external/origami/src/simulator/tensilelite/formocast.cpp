...
double getL1LoadRequest(double MTX,
  ...
  if (isTransposed) {
    ...
    if (L1CacheLineSize > MTX / NumLoadsCoalesced * bpe) tcc_ea0_coalesced = 2;
    if ((grvw * bpe == 8 || grvw * bpe <= 2) && (MTX / NumLoadsCoalesced * bpe >= L1BusWidthPerCU))
      L1_req *= 2;
    if (dtv) L1_req *= numWaveX;
  }
...
double getMultipleBufferOverhead(double M,
                                 double N,
                                 double GlobalSplitU,
                                 double NumBatches,
                                 uint32_t bpeCompute,
  ...
  double GSU_L2_clk = read_l2_req / WGs / 2 * 128 /
                      std::min(L2BandWidthPerCU_local, L2BusWidthPerCU) * scale_occupancy;
  ...
  double GSU_mem_overall =
      std::max(GSU_hbm_overall, std::max(GSU_L3_overall, std::max(GSU_L1_overall, GSU_L2_overall)));

  double D_L1_clk = write_l1_req / WGs * 64 / L1WriteBusWidthPerCU;
  double D_L2_clk =
      write_l2_req / WGs * 64 / std::min(L2WriteBusWidthPerCU, L2WriteBandWidthPerCU_local);
...
double getMultipleBufferSingleKernelOverhead(double GlobalSplitU,
  ...
  if (GlobalSplitU > 2) { GSU_L1_req += (MT0 * MT1 * bpeIn) / 64; }
  double GSU_L1_clk = GSU_L1_req * 64 / L1BusWidthPerCU * scale_occupancy;
  double GSU_L2_req = MT0 * MT1 * bpeIn * GlobalSplitU / 128;
  double GSU_L2_clk =
      GSU_L2_req * 128 / std::min(L2BandWidthPerCU_local, L2BusWidthPerCU) * scale_occupancy;

  double cost_overhead = 2 * 1024.0 / 1900 / (GlobalSplitU - 1);

  return storeGSU + atomic_overhead +
         ((std::max(GSU_L1_clk / cu_freq, GSU_L2_clk / cu_freq) + cost_overhead));
}

double getLocalSplitKOverhead(double MT0,
                              double MT1,
                              double lsu,
                              uint32_t svw,
                              uint32_t numThreads,
...
L1CacheHitRate computeL1CacheHitRate(double L1CacheCapacity,
                                     double L1CacheLineSize,
                                     double L1BusWidthPerCU,
                                     double MT0,
                                     double MT1,
  ...
  if (transA) {
    ...
    if (depthU / NLCA * bpeA < L1CacheLineSize) {
      bool isL1CacheFull =
          (L1CacheLineSize * threadnum / (depthU / NLCA / GRVWA)) > L1CacheCapacity;
      if (isL1CacheFull) {
        A_L1_hit = 1;
      } else {
    ...
    if (DTVA)
      A_L1_hit /=
          4 *
          ((MT0 * ceiling_math(depthU * bpeA, L1CacheLineSize) >= L1CacheCapacity) ? 1 : NumWave1);
  ...
  if (transB) {
    ...
    if ((GRVWB * bpeB == 8 || GRVWB * bpeB <= 2) && (MT1 / NLCB * bpeB) >= L1BusWidthPerCU)
      B_L1_hit /= 2;
    if (DTVB) B_L1_hit /= NumWave0;
    B_L1_hit = isL1BypassB ? 0 : 1 - B_L1_hit;
  } else {
    // B is N
    if (depthU / NLCB * bpeB < L1CacheLineSize) {
      bool isL1CacheFull =
          (L1CacheLineSize * threadnum / (depthU / NLCB / GRVWB)) > L1CacheCapacity;
      if (isL1CacheFull) {
        B_L1_hit = 1;
      } else {
    ...
    if (DTVB)
      B_L1_hit /=
          4 *
          ((MT1 * ceiling_math(depthU * bpeB, L1CacheLineSize) >= L1CacheCapacity) ? 1 : NumWave0);
    B_L1_hit = isL1BypassB ? 0 : 1 - B_L1_hit;
...
L3CacheHitRate computeL3CacheHitRate(double M,
                                     double N,
                                     double K,
...
L2CacheHitRate computeL2CacheHitRate(uint32_t M,
  ...
  if (isL2BypassA && isL2BypassB) return hitRate;
  ...
  if (WGMXCC > 0) assert((WGMXCCG % WGMXCC) == 0);
  ...
  // std::vector<uint32_t> xcd_cachedSizes(NumXCDs, 0); // Use this when cache-size-estimation is
  // better
  uint32_t xccIdx = 0;
  ...
  for (uint32_t wg = 0; wg < std::min(totalWGNum, 10 * NumCUs); wg++) {
    ...
    // |wgX-1| |     | |     | |     | |     |
    // |_____|_|_____|_|_____|_|_____|_|_____|  <---- Up to here, there are #-XCCG wgs. Note that
    // X*N = XCCG | wgY | |     | |     | |     | |     |  <---- Note that wgY = wg(XCCG-1)+1 | | |
    // | |     | |     | |     |
    //
    // ----------------------------------------------------------
    ...
    if (WGMXCCG == 0) {
      xccMappedWGId = (wg / WGMXCC) + (wg % WGMXCC) * (totalWGNum / WGMXCC) +
                      std::min((totalWGNum % WGMXCC), (wg % WGMXCC));
    } else {
  ...
  for (uint32_t xccIdx = 0; xccIdx < NumXCDs; xccIdx++) {
    ...
    // for(uint32_t xccMappedWGId : curXCDWGs)
    {
      ...
      // {
      //     // std::cout << "clean cache when: xccId = " << xccIdx << ", with remapped-wgID = " <<
      //     xccMappedWGId << std::endl; xcd_cachedSizes[xccIdx] = 0;
      //     xcd_cachedWG_A[xccIdx].clear();
      //     xcd_cachedWG_B[xccIdx].clear();
      // }
    }
  ...
  if (aHitElements > 0) hitRateA = double(aHitElements) / double(aHitElements + aMissElements);
  if (bHitElements > 0) hitRateB = double(bHitElements) / double(bHitElements + bMissElements);
...
double calculateStoreL3Request(double M,
                               double N,
                               double MT0,
                               double MT1,
                               double& non_edge_req,
...
double calculateStoreL2Request(double M,
                               double N,
                               double MT0,
                               double MT1,
                               double SVW,
  ...
  double non_edge_0 =
      MT0 * 2 / 64 * ceiling_math(64 / (16 * 2 * SVW)) *
      ((M_MOD_32 == 0 || M_MOD_32 == 16) ? (M_MOD_32 == 16 ? (SVW == 1   ? 1
                                                              : SVW == 2 ? (3.0 / 2)
                                                              : SVW == 4 ? (5.0 / 4)
                                                                         : (9.0 / 8))
                                                           : 1)
  ...
  double edge_3 =
      SVW == 1 ? 0
               : (M_MOD_8 == 0 ? 0 : std::floor(edge_size / (16 * SVW)) * (32 - SVW) / (32 / SVW));
  double edge_4 =
      SVW == 1 ? (M_MOD_4 == 0 ? 0 : ((std::fmod(edge_size, 32) - SVW) / (32 / SVW))) : 0;
  double edge_5 =
      SVW == 1 ? (M_MOD_4 == 0 ? 0 : (std::floor(edge_size / 32) * (32 - SVW) / (32 / SVW))) : 0;

  result = N * ((numWGsNonEdge * non_edge_0) + (edge_0) + (edge_1) + (edge_2) + (edge_3) +
                (edge_4) + (edge_5));
...
double calculateStoreL1Request(double M,
                               double N,
                               double MT0,
                               double MT1,
                               double SVW,
  ...
  double non_edge_0 = MT0 / 16 * (-1) * (SVW == 1 ? 1 : 0) * (std::fmod(M, 4) == 2 ? 1 : 0);
  double non_edge_1 = MT0 / 16 * (-4) * (SVW == 1 ? 1 : 0) * (std::fmod(M, 16) == 8 ? 1 : 0);
  double non_edge_2 = MT0 / 16 * (-3) * (SVW == 1 ? 1 : 0) * (std::fmod(M, 4) == 0 ? 1 : 0);
  double non_edge_3 = MT0 / 16 * (-12) * (SVW == 1 ? 1 : 0) * (std::fmod(M, 16) == 0 ? 1 : 0);
  ...
  double edge_3     = (std::floor(edge_size / (16 * SVW)) * 4 * (SVW == 1 ? 1 : 0) *
                   (std::fmod(M, 16) == 0 ? 1 : 0));
  double edge_4     = (std::floor(edge_size / (16 * SVW)) * (-12 * SVW * SVW) * (SVW == 1 ? 1 : 0) *
                   (std::fmod(M, 16) == 0 ? 1 : 0));

  result +=
      N / 64 *
      ((numWGsNonEdge * non_edge_0) + (numWGsNonEdge * non_edge_1) + (numWGsNonEdge * non_edge_2) +
       (numWGsNonEdge * non_edge_3) + (edge_0) + (edge_1) + (edge_2) + (edge_3) + (edge_4));
  ...
  double non_edge_8 = MT0 / 16 * (-4) * (SVW == 1 ? 0 : 1) * (std::fmod(M, 8) == 0 ? 1 : 0);
  ...
  double edge_5 = (std::floor(edge_size / (16 * SVW)) * (-16 * SVW * SVW) * (SVW == 1 ? 0 : 1) *
                   (std::fmod(M, 16 * SVW) == 8 * SVW ? 1 : 0));
  double edge_6 = (std::floor(edge_size / (16 * SVW)) * (-48 * SVW * SVW) * (SVW == 1 ? 0 : 1) *
                   (std::fmod(M, 16 * SVW) == 0 ? 1 : 0));

  result +=
      N / 64 *
      ((numWGsNonEdge * non_edge_4) + (numWGsNonEdge * non_edge_5) + (numWGsNonEdge * non_edge_6) +
       (numWGsNonEdge * non_edge_7) + (numWGsNonEdge * non_edge_8) + (numWGsNonEdge * non_edge_9) +
  ...
  double edge_7      = (std::floor(edge_size / (16 * SVW)) * (12 * SVW * SVW) * (SVW == 1 ? 1 : 4));
  double edge_8 =
      ((M_MOD_16SVW >= 4 * SVW ? (M_MOD_16SVW - 4 * SVW) * SVW *
  ...
  double maxMT1 = std::min(N, MT1);
  double nonEdgeRequestPerMT =
      maxMT1 / 64 *
          (non_edge_0 + non_edge_1 + non_edge_2 + non_edge_3 + non_edge_4 + non_edge_5 +
           non_edge_6 + non_edge_7 + non_edge_8 + non_edge_9 + non_edge_10 + non_edge_11) +
      (maxMT1 * non_edge_12);
...
int getGlobalReadQueueFullStallCycles(int currentCycle,
                                      std::deque<int>& fifo,
                                      int bpRead,
                                      int numWaves,
                                      bool isStall,
  ...
  if (isSgprOffset) { extraIssueCycles = 1; }
  if (!isStall) { return currentCycle + extraIssueCycles; }
  int finalCycle = currentCycle;
  // GR FIFO length is 16, stall cycles is 16 cycles.
  const int grFIFOLength = 16;
  int grStallCycles      = 4;
  if (bpRead <= 4) { grStallCycles = 1; }
  ...
  while (fifo.size() > maxRetainedEntries) { fifo.pop_front(); }
  // pop finished GRs
  if (fifo.size() < grFIFOLength) {
    // if FIFO is not empty, set finalCycle to the last cycle + 1. Only 1 GR can be issued in each
    // cycle.
    if (fifo.size() > 0) {
    ...
    if (intervalOfGRs > 4) { readStalledLength += (intervalOfGRs - 4) * numWaves; }
...
int getLocalReadQueueFullStallCycles(int currentCycle,
                                     std::queue<int>& fifo,
                                     int bpRead,
                                     int numWaves,
...
/**
 * @brief Calculate local read latency based on base latency, conflict multiplier, and bank
 * conflicts
 * @param baseLatency Base latency for local read operation (from HardwareConstants:
 * LocalReadBaseLatencyB128/B64/B32)
 * @param conflictMultiplier Multiplier applied to bank conflict penalty (from HardwareConstants:
 * LocalReadConflictMultiplierB128/B64/B32)
...
/**
 * @brief Calculate local write latency based on base latency, conflict multiplier, and bank
 * conflicts
 * @param baseLatency Base latency for local write operation (from HardwareConstants:
 * LocalWriteBaseLatencyB128/B64/B32)
 * @param conflictMultiplier Multiplier applied to bank conflict penalty (from HardwareConstants:
 * LocalWriteConflictMultiplierB128/B64/B32)
 * @param bankConflict Bank conflict factor (typically 1.0 for no conflict, >1.0 for conflicts)
 * @return Calculated latency in cycles (baseLatency + conflict penalty based on bank conflicts)
 */
...
double analyzeBankConflictsFromVGPR(
  ...
  if (!vgprLocalReadAddrA.empty()) {
    for (int tid = 0; tid < NUM_THREADS_TO_SIMULATE; tid++) {
      if (!vgprLocalReadAddrA.empty() &&
          vgprState[tid].find(vgprLocalReadAddrA) != vgprState[tid].end()) {
