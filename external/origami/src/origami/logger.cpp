...
Logger::~Logger() {
  ...
  if (format_ == LogFormat::TEXT) {
    if (log_file_.is_open()) {
      log_file_ << "[INFO ] " << "logger.cpp" << ":" << __LINE__ << " - Logger shutting down"
                << std::endl;
      log_file_.close();
    }
  } else {
...
void Logger::log(LogLevel level, const std::string& message, const char* file, int line) {
  ...
  if (format_ == LogFormat::TEXT) {
    ...
    log_file_ << "[" << level_to_string(level) << "] " << filename << ":" << line << " - "
              << message << std::endl;
...
void Logger::update_from_env() {
  ...
  if (enabled_.load(std::memory_order_relaxed)) {
    if (format_ == LogFormat::CSV && !rows_.empty()) { flush_csv_locked(); }
  }
...
void Logger::process_debug_message(const std::string& message) {
  ...
  if (pos != std::string::npos) { record(message.substr(0, pos), message.substr(pos + 2)); }
}
...
void Logger::record(const std::string& key, const std::string& value) {
  if (tl_csv_state.row_in_progress) { tl_csv_state.current_row.emplace_back(key, value); }
}
...
void Logger::end_row() {
  ...
  for (const auto& [key, value] : tl_csv_state.current_row) { row[column_index_[key]] = value; }
...
void Logger::flush_csv_locked() {
  ...
  if (!file.is_open()) {
    std::cerr << "Warning: Failed to open CSV file: " << file_path_ << ". Dropping " << rows_.size()
              << " buffered row(s)." << std::endl;
    rows_.clear();
    return;
  }
  ...
  for (const auto& row : rows_) {
    for (size_t i = 0; i < columns_.size(); ++i) {
      if (i > 0) file << ",";
      if (i < row.size()) { file << escape_csv(row[i]); }
    }
    file << "\n";
  }
