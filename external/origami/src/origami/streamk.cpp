...
size_t grid_k_split_aware(const problem_t& problem,
  ...
  auto causes_partial_cachelines = [&](size_t candidate_grid) -> bool {
    ...
    const size_t a_contig_bytes =
        (problem.a_transpose == transpose_t::T) ? fragment_iters * config.mt.k * bpe_a : 0;
    const size_t b_contig_bytes =
        (problem.b_transpose == transpose_t::N) ? fragment_iters * config.mt.k * bpe_b : 0;
  ...
  if (tiles > cu_count) {
    ...
    for (double frac : tile_fractions) {
      ...
      if (causes_partial_cachelines(frac_grid)) continue;
...
hybrid_mode_t select_hybrid_mode(const problem_t& problem,
  ...
  if (hardware.arch != hardware_t::architecture_t::gfx950) return hybrid_mode_t::static_;

  const size_t MT_M  = config.mt.m;
  const size_t MT_N  = config.mt.n;
  const size_t batch = std::max<size_t>(problem.batch, 1);
  const size_t tiles =
      compute_number_of_output_tiles(MT_M, MT_N, problem.size.m, problem.size.n, batch);
  ...
  size_t available_cus =
      (sm_count_target > 0) ? std::min<size_t>(sm_count_target, hardware.N_CU) : hardware.N_CU;
  ...
  const double tiles_per_cu = static_cast<double>(tiles) / static_cast<double>(available_cus);
