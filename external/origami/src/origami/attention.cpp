...
std::tuple<size_t, size_t, size_t, size_t> compute_cu_occupancy(const problem_t& problem,
                                                                ...
                                                                size_t max_cus,
                                                                size_t split = 0) {
  OLOG_DEBUG("compute_cu_occupancy: problem size (M="
             << problem.size.m << ", N=" << problem.size.n << ", K=" << problem.size.k
             << "), config MT (M=" << config.mt.m << ", N=" << config.mt.n << ", K=" << config.mt.k
             << ")");
...
bool check_rf_capacity(const hardware_t& hardware, dim3_t mt, data_type_t a_dtype) {
  // RF size: Q[BLOCK_M, HEAD_DIM] in F16, K and V[BLOCK_N, HEAD_DIM] in F16, O[BLOCK_M, HEAD_DIM]
  // in F32, and P[BLOCK_M, BLOCK_N] in F32 for 2 WGs
...
bool check_lds_capacity(const hardware_t& hardware, dim3_t mt, data_type_t a_dtype) {
  auto lds_usage = 2 * std::max(mt.mk(), mt.nk()) *
                   data_type_to_bytes(a_dtype);  // max(Q, K), V reuses the LDS space for K
...
double estimate_l2_hit(const problem_t& problem,
  ...
  size_t l2_tile_n =
      std::min(static_cast<size_t>(std::max(config.workgroup_mapping, 1)), workgroups_n);
...
double estimate_mall_hit(const problem_t& problem,
  ...
  size_t mall_tile_m = math::safe_ceil_div(
      num_active_cus, static_cast<size_t>(std::max(config.workgroup_mapping, 1)));
  size_t mall_tile_n =
      std::min(static_cast<size_t>(std::max(config.workgroup_mapping, 1)), workgroups_n);
...
static double q_tile_load_latency(const problem_t& problem,
  ...
  const double a_bytes = data_type_to_bytes(problem.a_dtype);
  double bytes_per_cu =
      static_cast<double>(config.mt.m) * static_cast<double>(config.mt.k) * a_bytes;
...
static double kv_tile_load_latency(const problem_t& problem,
  ...
  double Ld_CU_bytes =
      static_cast<double>(config.mt.n) * static_cast<double>(config.mt.k) * b_bytes;
...
static double output_tile_write_latency(const problem_t& problem,
  ...
  double bytes_per_cu =
      static_cast<double>(config.mt.m) * static_cast<double>(config.mt.k) * d_bytes;
  double total_bytes = bytes_per_cu * static_cast<double>(num_active_cus);
 ...
 *
 *        prologue   t0                    t1                    t2            t3          t4
 * epilogue
 *       +----------+---------------------+--------------------+--------------+-----------+-----------+-----------+
 * WG 0: | [LD Q K] | [WGMMA0 + Softmax] | [LD V]            | [WGMMA1]     | [ST O]    | [LD Q K]
 * |           | WG 1: |          | [LD Q K]            | [WGMMA0 + Softmax]| [LD V]       |
 * [WGMMA1]  | [ST O]    | [ST O]    |
 *       +----------+---------------------+--------------------+--------------+-----------+-----------+-----------+
 */
double compute_tile_latency(const problem_t& problem,
  ...
  double L_ld_kv =
      kv_tile_load_latency(problem, hardware, config, num_active_cus, splitting_factor);
  ...
  if (debug) {
    ...
    OLOG_DEBUG("    WGMMA0 (Q*K^T): MT_M=" << config.mt.m << " MT_N=" << config.mt.n
                                           << " MT_K=" << config.mt.k << " MI=(" << config.mi.m
                                           << "," << config.mi.n << "," << config.mi.k << ")"
                                           << " latency=" << L_wgmma0 << " cycles");
    OLOG_DEBUG("    Softmax: elements=" << (config.mt.m * config.mt.n) << " latency=" << L_softmax
                                        << " cycles");
    OLOG_DEBUG("    WGMMA1 (P*V): remapped dims M=" << config.mt.m << " N=" << config.mt.k << " K="
                                                    << config.mt.n << " MI=(" << config.mi.m << ","
                                                    << config.mi.n << "," << config.mi.k << ")"
  ...
  if (debug) { OLOG_DEBUG("  Loop cycle (critical path): " << loop_cycle << " cycles"); }
...
double compute_total_latency(const problem_t& problem,
  ...
  OLOG_DEBUG("Evaluating config: MT=(" << config.mt.m << "," << config.mt.n << "," << config.mt.k
                                       << ") MI=(" << config.mi.m << "," << config.mi.n << ","
                                       << config.mi.k << ")");
  OLOG_DEBUG("Problem: M=" << problem.size.m << " N=" << problem.size.n << " K=" << problem.size.k
  ...
  */
  ...
  OLOG_DEBUG("Grid: M=" << M << "/" << MT_M << "=" << grid_m << " N=" << N << "/" << MT_N << "="
                        << grid_n << " K=" << K << "/" << MT_K << "=" << grid_k
  ...
  double L_loop = compute_tile_latency(problem, hardware, config, num_active_cus, splitting_factor);
  ...
  const size_t adjustment_factor =
      8;  // adjusting amount of work for prolog or epilogue bound tile sizes
  double total_tiles = static_cast<double>(adjustment_factor) * static_cast<double>(batch) *
                       static_cast<double>(q_heads) * static_cast<double>(grid_m) *
                       static_cast<double>(grid_n) * static_cast<double>(grid_k);
  double num_iters = std::ceil(
      total_tiles / (static_cast<double>(wgs_per_cu) * static_cast<double>(num_active_cus)));
  num_iters = std::max(num_iters, 1.0);

  // --- Total latency ---
  double total_latency = L_prologue + L_loop * num_iters + L_epilogue;

  OLOG_DEBUG("Pipeline summary: prologue=" << L_prologue << " loop=" << L_loop << " epilogue="
                                           << L_epilogue << " num_iters=" << num_iters
