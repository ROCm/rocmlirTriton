...
Formocast::ProblemInfo make_problem_info(double M,
                                         double N,
                                         double K,
                                         uint32_t bpeA        = 2,
                                         uint32_t bpeB        = 2,
                                         uint32_t bpeD        = 2,
                                         uint32_t bpeCompute  = 2,
                                         bool transA          = true,
                                         bool transB          = false,
                                         double numBatches    = 1,
...
Formocast::SizeMapping make_size_mapping(int MT0          = 128,
                                         int MT1          = 128,
                                         int depthU       = 32,
                                         int miM          = 32,
                                         int miN          = 32,
...
TEST_CASE("Formocast: Hardware constants retrieval", "[formocast]") {
  ...
  SECTION("Unsupported architecture throws exception") {
    REQUIRE_THROWS_AS(simulator.getHardwareConstants(static_cast<hardware_t::architecture_t>(9999)),
                      std::runtime_error);
  }
}
...
TEST_CASE("Formocast: Cache hit rate computation", "[formocast]") {
  ...
  SECTION("L1 cache hit rate computation") {
    auto l1_hit = simulator.computeL1CacheHitRate(hw,
                                                  128,
                                                  128,
                                                  32,
                                                  2,
  ...
  SECTION("L2 cache hit rate computation") {
    ...
    auto l2_hit =
        simulator.computeL2CacheHitRate(1024, 1024, 1024, hw, 8, 256, 1, 8, 1, 2, 2, 0, 0, false);
  ...
  SECTION("L3 cache hit rate computation") {
    auto l3_hit = simulator.computeL3CacheHitRate(1024, 1024, 1024, hw, 2, 2, 0, 0, 8, 8, 8, 8);
...
TEST_CASE("Formocast: Tie-breaker comparison", "[formocast]") {
  ...
  SECTION("Standalone tie-breaker function for skinny N") {
    ...
    bool result = compareConfigTieBreaker(1024,
                                          32,
                                          1024,
  ...
  SECTION("Standalone tie-breaker function for skinny M") {
    ...
    bool result = compareConfigTieBreaker(32,
                                          1024,
                                          1024,
                                          1,  // problem dimensions
                                          32,
  ...
  SECTION("Tie-breaker returns false for equal configs") {
    bool result = compareConfigTieBreaker(1024, 1024, 1024, 1, 128, 128, 32, 4, 128, 128, 32, 4);
...
TEST_CASE("Formocast: GSU overhead calculation", "[formocast]") {
  ...
  SECTION("GSU overhead with MultipleBuffer method") {
    double gsu_overhead =
        simulator.calculateGlobalSplitUOverhead(1024,
                                                1024,
                                                1024,
                                                1,
                                                2,
                                                2,  // gsuMethod=2 (MultipleBuffer)
                                                problem,
                                                hw,
                                                1,
                                                1,
  ...
  SECTION("GSU overhead with MBSK method") {
    double gsu_overhead = simulator.calculateGlobalSplitUOverhead(1024,
                                                                  1024,
                                                                  1024,
                                                                  1,
                                                                  2,
  ...
  SECTION("GSU overhead with small M and N values") {
    ...
    double gsu_overhead = simulator.calculateGlobalSplitUOverhead(
        ...
        1,
        2,
        2,  // M=2, N=4, K=1024, GlobalSplitU=2, MultipleBuffer
        problem,
        hw,
        1,
        1,
...
TEST_CASE("Formocast: LSU overhead calculation", "[formocast]") {
  ...
  SECTION("LSU overhead calculation") {
    double lsu_overhead = simulator.calculateLocalSplitUOverhead(128, 128, 2, 4, 256, problem, hw);

    REQUIRE(lsu_overhead == Approx(1.066667).epsilon(0.01));
  }

  SECTION("LSU overhead increases with larger LSU value") {
    double lsu_overhead1 = simulator.calculateLocalSplitUOverhead(128, 128, 2, 4, 256, problem, hw);

    double lsu_overhead2 = simulator.calculateLocalSplitUOverhead(128, 128, 4, 4, 256, problem, hw);
...
TEST_CASE("Formocast: Memory access costs calculation", "[formocast]") {
  ...
  SECTION("Calculate memory access costs") {
    auto mem_costs = simulator.calculateMTMemoryAccessCosts(1024,
                                                            1024,
                                                            256,
                                                            128,
                                                            128,
