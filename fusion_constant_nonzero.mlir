// Fused GEMM test: C = (A + 1) * (B + 1) + 1
// Tests constant fusion handling (NON-zero-preserving case).
// Two issues: (1) arith.constant does not trace to a function argument,
// so InsertLoads does NOT insert rock.load for it, and LowerLoads may fail.
// (2) Adding a non-zero constant to padded GEMM inputs corrupts the padded
// region (padding should be zero for correctness, but becomes 1.0), leading
// to wrong GEMM results.
