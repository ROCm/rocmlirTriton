...
// Tests cascaded type-changing output fusion.
...
module attributes {rock.arch = "amdgcn-amd-amdhsa:gfx1100"} {
  ...
  func.func @main() {
    ...
    call @_init_buffer_f16(%alloc_A) : (memref<10000xf16>) -> ()
    ...
    call @_init_buffer_f16(%alloc_B) : (memref<10000xf16>) -> ()
    %alloc_B_ref = memref.alloc() : memref<10000xf16>
    memref.copy %alloc_B, %alloc_B_ref : memref<10000xf16> to memref<10000xf16>

    %alloc_bias = memref.alloc() : memref<10000xf16>
    call @_init_buffer_f16(%alloc_bias) : (memref<10000xf16>) -> ()
    %alloc_bias_ref = memref.alloc() : memref<10000xf16>
    memref.copy %alloc_bias, %alloc_bias_ref : memref<10000xf16> to memref<10000xf16>

    %alloc_C = memref.alloc() : memref<10000xf32>
    %cst_zero = arith.constant 0.000000e+00 : f32
    linalg.fill ins(%cst_zero : f32) outs(%alloc_C : memref<10000xf32>)
    %alloc_C_ref = memref.alloc() : memref<10000xf32>
    linalg.fill ins(%cst_zero : f32) outs(%alloc_C_ref : memref<10000xf32>)

    call @rock_gemm_gpu(%alloc_A, %alloc_B, %alloc_bias, %alloc_C) : (memref<10000xf16>, memref<10000xf16>, memref<10000xf16>, memref<10000xf32>) -> ()
    call @host_naive_fused_gemm(%alloc_A_ref, %alloc_B_ref, %alloc_bias_ref, %alloc_C_ref) : (memref<10000xf16>, memref<10000xf16>, memref<10000xf16>, memref<10000xf32>) -> ()
    ...
    memref.dealloc %alloc_bias_ref : memref<10000xf16>
    ...
    memref.dealloc %alloc_bias : memref<10000xf16>
    memref.dealloc %alloc_C : memref<10000xf32>
    return
  }

  func.func @_init_buffer_f16(%buf: memref<10000xf16>) {
  ...
  // CPU reference: C_f32 = extf(gemm(A_f16, B_f16) + bias_f16)
  func.func @host_naive_fused_gemm(%arg0: memref<10000xf16>, %arg1: memref<10000xf16>, %arg2: memref<10000xf16>, %arg3: memref<10000xf32>) {
    ...
    %bias_f32 = memref.alloc() : memref<10000xf32>
    call @_memcpy_f16_f32_10000(%arg2, %bias_f32) : (memref<10000xf16>, memref<10000xf32>) -> ()
    %C_f32 = memref.alloc() : memref<10000xf32>

    // GEMM in f32 (matching GPU internal accumulation)
    %cst_zero = arith.constant 0.000000e+00 : f32
    ...
    // Output fusion: truncate GEMM to f16, add bias in f16, then extend to f32
    ...
    scf.for %i = %c0 to %c10000 step %c1 {
      ...
      %b_val = memref.load %bias_f32[%i] : memref<10000xf32>
      %b_f16 = arith.truncf %b_val : f32 to f16
      %fused_f16 = arith.addf %c_f16, %b_f16 : f16
      %fused_f32 = arith.extf %fused_f16 : f16 to f32
      memref.store %fused_f32, %C_f32[%i] : memref<10000xf32>
    }

    memref.copy %C_f32, %arg3 : memref<10000xf32> to memref<10000xf32>
    memref.dealloc %A_f32 : memref<10000xf32>
    memref.dealloc %B_f32 : memref<10000xf32>
    memref.dealloc %bias_f32 : memref<10000xf32>
  ...
  func.func @rock_gemm_gpu(%arg0: memref<10000xf16>, %arg1: memref<10000xf16>, %arg2: memref<10000xf16>, %arg3: memref<10000xf32>) {
    ...
    %memref_bias = gpu.alloc  () : memref<10000xf16>
    gpu.memcpy  %memref_bias, %arg2 : memref<10000xf16>, memref<10000xf16>
    %memref_C = gpu.alloc  () : memref<10000xf32>
    gpu.memcpy  %memref_C, %arg3 : memref<10000xf32>, memref<10000xf32>
    ...
    %2 = bufferization.to_tensor %memref_bias restrict writable : memref<10000xf16> to tensor<10000xf16>
    %3 = bufferization.to_tensor %memref_C restrict writable : memref<10000xf32> to tensor<10000xf32>

    %4 = call @rock_gemm(%0, %1, %2, %3) : (tensor<10000xf16>, tensor<10000xf16>, tensor<10000xf16>, tensor<10000xf32>) -> tensor<10000xf32>

    %5 = bufferization.to_buffer %4 : tensor<10000xf32> to memref<10000xf32>
    memref.copy %5, %memref_C : memref<10000xf32> to memref<10000xf32>
    ...
    gpu.memcpy  %arg2, %memref_bias : memref<10000xf16>, memref<10000xf16>
    gpu.dealloc  %memref_bias : memref<10000xf16>
    gpu.memcpy  %arg3, %memref_C : memref<10000xf32>, memref<10000xf32>
