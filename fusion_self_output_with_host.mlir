// Complex output fusion DAG with host verification:
//   gemm [1,100,100]
//   T1 = Unmerge{10,10} on n → [1,100,10,10]
//   exp(T1_result) in 4D
//   mulf(T1_result, exp_result) in 4D
//   T2 = Merge{10,10} on (n0,n1) → [1,100,100]
//   addf(T2_result, gemm) in 3D
...
module attributes {rock.arch = "amdgcn-amd-amdhsa:gfx1100"} {
  ...
  // ---- CPU reference: C[i] = gemm(A,B)[i] * exp(gemm(A,B)[i]) + gemm(A,B)[i] ----
  func.func @host_naive_fused_gemm(%arg0: memref<10000xf16>, %arg1: memref<10000xf16>, %arg2: memref<10000xf16>) {
    ...
    // Output fusion: C[i] = C[i] * exp(C[i]) + C[i]
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %c10000 = arith.constant 10000 : index
    scf.for %i = %c0 to %c10000 step %c1 {
      %c_val = memref.load %C_f32[%i] : memref<10000xf32>
      %e_val = math.exp %c_val : f32
      %mul_val = arith.mulf %c_val, %e_val : f32
      %fused_val = arith.addf %mul_val, %c_val : f32
      memref.store %fused_val, %C_f32[%i] : memref<10000xf32>
    }

    call @_memcpy_f32_f16(%C_f32, %arg2) : (memref<10000xf32>, memref<10000xf16>) -> ()
